...
export default function StockAlerts() {
  ...
  const filteredProducts = useMemo(() => {
    ...
    return list.filter((p) =>
      searchMatchesAllTerms(search, p.product_name, p.sku, p.category, p.group_name),
    );
  }, [data, search]);
  ...
  const filteredSoles = useMemo(() => {
    ...
    if (!search.trim()) return list;
    return list.filter((p) =>
      searchMatchesAllTerms(search, p.product_name, p.sku, p.group_name, p.size),
    );
  }, [data, search]);
        ...
        <Panel
          ...
          actions={
            <SearchInput
              value={search}
              onChange={setSearch}
              placeholder="Buscar por produto, SKU, categoria, grupo…"
              resultCount={filteredProducts.length + filteredSoles.length}
              totalCount={(data?.products.length ?? 0) + (data?.soleSizes.length ?? 0)}
              className="w-72"
