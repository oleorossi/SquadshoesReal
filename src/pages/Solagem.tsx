...
export default function Solagem() {
  ...
  const hasPendingSolagemStages = useMemo(() => {
    return allStages.some(stage => sameStage(stage.stage_name, 'Solagem') && (stage.status === 'pendente' || stage.status === 'em_andamento'));
  }, [allStages]);
        ...
        {soleData.length === 0 ? (
          <Panel flush>
            <EmptyState
              ...
              description="As demandas aparecerão aqui conforme as OPs entrarem no setor de Solagem."
            />
          </Panel>
        ) : (
          <Panel
            eyebrow="PRODUÇÃO · SOLAGEM"
