...
import {
  ...
  CalendarDays, BarChart3, Layers, TrendingDown,
} from 'lucide-react';
...
function computeWindows(sheet: any, qty: number, deadline: Date, defaults: any | null) {
  const lt  = (s: SectorKey) => computeSectorLeadTimeDays(s, qty, sheet, defaults);
  const on  = (s: SectorKey) => hasSector(sheet, s);
  const cap = (f: string) => { const v = Number(sheet?.[f] || 0); return v > 0 ? v : Number(defaults?.[f] || 0); };
  ...
  const acabEnd=deadline; const acabStart=addBizDays(acabEnd,-ltAcab);
  const solaEnd=acabStart; const solaStart=addBizDays(solaEnd,-ltSola);
  const montEnd=solaStart; const montStart=addBizDays(montEnd,-ltMont);
  const colaEnd=montStart; const colaStart=addBizDays(colaEnd,-ltCola);
  const silkEnd=colaStart; const silkStart=addBizDays(silkEnd,-ltSilk);
  const mesaEnd=silkStart; const mesaStart=addBizDays(mesaEnd,-ltMesa);
  const forrEnd=mesaStart; const forrStart=addBizDays(forrEnd,-ltForr);
  const palmEnd=forrStart; const palmStart=addBizDays(palmEnd,-ltPalm);

  type W = { start: Date; end: Date; cap: number };
  const w: Partial<Record<SectorKey, W>> = {};
