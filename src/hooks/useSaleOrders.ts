...
export type SaleOrderItemFormData = {
  ...
  strap_colors?: Array<{
    ...
    /** Escolha do PV quando Hub = escolhe_no_pv (fábrica | prestador | tira pronta). */
    pv_origem?: 'fabrica' | 'prestador' | 'sku_acabado' | null;
  }>;
...
export function useUpdateSaleOrderStatus(options?: {
  ...
  return useMutation({
    ...
    onSuccess: (engineResult, vars) => {
      ...
      qc.invalidateQueries({ queryKey: ['sale_orders_floor_progress'] });
      qc.invalidateQueries({ queryKey: ['accounts_receivable'] });
      qc.invalidateQueries({ queryKey: ['financial_entries'] });
...
export function useResyncOPsFromSheets() {
  ...
  return useMutation({
    ...
    onSuccess: (result) => {
      [
        ['sale_orders'], ['orders'], ['order_stages'], ['sale_orders_floor_progress'], ['products'],
