...
describe('stageFlow · bloqueios rígidos de corte', () => {
  it('não inicia Acabamento Palmilha antes de Corte Fibra entregar pares', () => {
    const stages = [stage('Corte Fibra'), stage('Acabamento Palmilha')];
    expect(findBlockingStage('Acabamento Palmilha', stages)?.stage_name).toBe('Corte Fibra');
  });

  it('libera Acabamento Palmilha quando Corte Fibra apontou produção parcial', () => {
    const stages = [stage('Corte Fibra', 1, 'em_andamento'), stage('Acabamento Palmilha')];
    expect(findBlockingStage('Acabamento Palmilha', stages)).toBeNull();
  });
...
describe('stageFlow · pulo não entrega o total da OP', () => {
  it('Colagem pulada (0 processados) não libera Montagem com o total', () => {
    const stages = [
      ...
      stage('Acabamento Palmilha', 180, 'concluido', 288),
    ...
    // min(Corte Fibra 180, Acabamento Palmilha 180, Costura Cabedal 0) = 0
