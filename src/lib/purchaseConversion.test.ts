...
describe('effectiveConversionFactor — área (m linear → dm²) pela largura da ficha', () => {
  it('m → dm² usa 10 × largura(dm)', () => {
    ...
    expect(effectiveConversionFactor({ unit: 'dm²', purchase_unit: 'm', conversion_rate: 100, dimensions_width: 10, dimensions_unit: 'dm' })).toBe(100);
    // largura 14 dm (= 1,4 m) → 140 dm²
    expect(effectiveConversionFactor({ unit: 'dm²', purchase_unit: 'm', conversion_rate: 140, dimensions_width: 14, dimensions_unit: 'dm' })).toBe(140);
  });

  it('m → m² usa largura(dm)/10', () => {
    expect(effectiveConversionFactor({ unit: 'm²', purchase_unit: 'm', conversion_rate: 1, dimensions_width: 10, dimensions_unit: 'dm' })).toBe(1);
  });
  ...
  it('REGRESSÃO: sinônimo "metro"/"metros"/"mt" roteia igual a "m" (não infla ~100×)', () => {
    ...
    expect(effectiveConversionFactor({ unit: 'dm²', purchase_unit: 'metro', conversion_rate: 100, dimensions_width: 10, dimensions_unit: 'dm' })).toBe(100);
    expect(effectiveConversionFactor({ unit: 'dm²', purchase_unit: 'metros', conversion_rate: 100, dimensions_width: 10, dimensions_unit: 'dm' })).toBe(100);
    expect(effectiveConversionFactor({ unit: 'dm²', purchase_unit: 'mt', conversion_rate: 100, dimensions_width: 10, dimensions_unit: 'dm' })).toBe(100);
  });

  it('REGRESSÃO: unidade de estoque "dm2"/"m2" (sem expoente) normaliza pra área', () => {
    expect(effectiveConversionFactor({ unit: 'dm2', purchase_unit: 'm', conversion_rate: 100, dimensions_width: 10, dimensions_unit: 'dm' })).toBe(100);
    expect(effectiveConversionFactor({ unit: 'm2', purchase_unit: 'm', conversion_rate: 1, dimensions_width: 10, dimensions_unit: 'dm' })).toBe(1);
  });
});
...
describe('effectiveConversionFactor — múltiplos diretos e conversion_rate', () => {
  ...
  it('exige conversion_rate válido quando as unidades divergem', () => {
    expect(effectiveConversionFactor({ unit: 'g', purchase_unit: 'kg' })).toBeNull();
    expect(effectiveConversionFactor({ unit: 'g', purchase_unit: 'kg', conversion_rate: 0 })).toBeNull();
    expect(effectiveConversionFactor({ unit: 'g', purchase_unit: 'kilo', conversion_rate: 1000 })).toBe(1000);
    expect(effectiveConversionFactor({ unit: 'ml', purchase_unit: 'litro', conversion_rate: 1000 })).toBe(1000);
  });
});
...
describe('effectiveConversionFactorStrict — bloqueio quando não há regra segura', () => {
  ...
  it('retorna a largura quando ela existe com unidade explícita (não bloqueia)', () => {
    expect(effectiveConversionFactorStrict({ unit: 'dm²', purchase_unit: 'm', conversion_rate: 100, dimensions_width: 10, dimensions_unit: 'dm' })).toBe(100);
  });

  it('bloqueia largura sem unidade, mesmo com conversion_rate válido', () => {
    expect(effectiveConversionFactorStrict({ unit: 'dm²', purchase_unit: 'm', conversion_rate: 100, dimensions_width: 10 })).toBeNull();
  });
...
describe('purchaseToStock', () => {
  it('multiplica a qtd de compra pelo fator efetivo', () => {
    ...
    expect(purchaseToStock(13.7, { unit: 'dm²', purchase_unit: 'm', conversion_rate: 100, dimensions_width: 10, dimensions_unit: 'dm' })).toBeCloseTo(1370, 6);
    // 14 kg = 14.000 g
    expect(purchaseToStock(14, { unit: 'g', purchase_unit: 'kg', conversion_rate: 1000 })).toBe(14000);
  });
});
