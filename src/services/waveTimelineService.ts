    ...
    if (existingPo) {
      await executePurchaseOrderCommand({
        ...
        payload: {
          header_patch: {
            notes: waveRef,
            linked_sale_order_ids_add: waveSaleOrderIds,
            source_pv_ids_add: waveSaleOrderIds,
            purchase_by_date_min: groupPurchaseDeadline,
          },
          items: poItems,
        },
        logicalKey: purchaseOrderLogicalKey('wave-append', waveId, supplierKey),
      });
    } else {
      const idempotencyKey = `wave:${waveId}:${supplierKey}`;
      const result = await executePurchaseOrderCommand({
        ...
        logicalKey: purchaseOrderLogicalKey('wave-create', waveId, supplierKey),
      });
      if (!result.deduplicated) posCreated++;
    }
  }
