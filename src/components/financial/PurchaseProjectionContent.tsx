      ...
      <div className="grid grid-cols-2 md:grid-cols-5 gap-3">
        <Card>
          <CardContent className="p-3">
            <div className="text-xs uppercase text-muted-foreground font-bold">Produtos ativos</div>
            <div className="text-2xl font-bold tabular-nums mt-0.5">
              {summary ? fmtBR(summary.total_active_products) : <Skeleton className="h-7 w-12" />}
            </div>
            <div className="text-xs text-muted-foreground mt-0.5">com compra ou consumo</div>
          </CardContent>
        </Card>
        <Card className="border-red-500/30">
          <CardContent className="p-3">
            ...
            <div className="text-xs text-muted-foreground mt-0.5">vão zerar antes do lead time</div>
          </CardContent>
        </Card>
        <Card className="border-amber-500/30">
          <CardContent className="p-3">
            <div className="text-xs uppercase text-amber-600 font-bold">Repor em breve</div>
            <div className="text-2xl font-bold tabular-nums mt-0.5">
              {summary ? summary.reorder_count : <Skeleton className="h-7 w-12" />}
            </div>
            <div className="text-xs text-muted-foreground mt-0.5">cobertura &lt; 1.5× lead time</div>
          </CardContent>
        </Card>
        <Card className="border-blue-500/30">
          <CardContent className="p-3">
            <div className="text-xs uppercase text-blue-600 font-bold flex items-center gap-1">
              <TrendingDown className="w-3 h-3" /> Estoque alto
            </div>
            <div className="text-2xl font-bold tabular-nums mt-0.5">
              {summary ? summary.high_stock_count : <Skeleton className="h-7 w-12" />}
            </div>
            <div className="text-xs text-muted-foreground mt-0.5">mais de 90 dias parados</div>
          </CardContent>
        </Card>
        <Card>
          <CardContent className="p-3">
            <div className="text-xs uppercase text-muted-foreground font-bold">Reposição estimada</div>
            <div className="text-2xl font-bold tabular-nums mt-0.5">
              {summary ? fmtMoney(summary.total_reorder_value) : <Skeleton className="h-7 w-20" />}
            </div>
            <div className="text-xs text-muted-foreground mt-0.5">valor a comprar p/ críticos+repor</div>
          </CardContent>
        </Card>
      </div>
  ...
  return (
    <TableRow className="hover:bg-muted/40">
      <TableCell className="px-3 py-2">
        <Badge className={cn('font-bold w-6 h-6 flex items-center justify-center p-0 text-xs', ABC_CLASS_BG[row.abc_class as AbcClass])}>
          {row.abc_class}
        </Badge>
      </TableCell>
      <TableCell className="px-3 py-2 max-w-[220px]">
        ...
        {row.is_artisanal && (
          <Badge variant="outline" className="text-xs mt-0.5 bg-amber-500/10 text-amber-700 border-amber-500/30">Artesanal</Badge>
        )}
      </TableCell>
      <TableCell className="px-3 py-2 text-xs">
        {row.color ? <Badge variant="outline" className="text-xs">{row.color}</Badge> : <span className="text-muted-foreground">—</span>}
      </TableCell>
      ...
      <TableCell className="px-3 py-2">
        <Badge variant="outline" className={cn('text-xs gap-1', RECOMMENDATION_CLASS[row.recommendation as Recommendation])}>
