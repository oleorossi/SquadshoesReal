...
export interface HubTab {
  ...
  icon?: LucideIcon;
  badge?: number | string;
  /** Agrupamento visual opcional para hubs com etapas de trabalho distintas. */
  group?: string;
}
...
export function HubTabsList({ tabs, className, size = 'sm', ariaLabel = 'Seções da página' }: HubTabsListProps) {
  return (
    ...
    >
      ...
      >
        {tabs.map((tab, index) => {
          const beginsGroup = Boolean(tab.group) && tab.group !== tabs[index - 1]?.group;
          return (
            <Fragment key={tab.value}>
              {beginsGroup && index > 0 && (
                <span role="presentation" aria-hidden="true" className="mx-1 h-5 w-px shrink-0 self-center bg-border" />
              )}
              {beginsGroup && (
                <span
                  role="presentation"
                  aria-hidden="true"
                  className="flex min-h-8 shrink-0 items-center px-2 font-mono text-[9px] font-bold uppercase tracking-[0.14em] text-muted-foreground/80"
                >
                  {tab.group}
                </span>
              )}
