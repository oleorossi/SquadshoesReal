// RelatorioAtrasos — primeiro relatório da Folha: resumo de ATRASOS por
// funcionário no período. Mostra, de forma simplificada, em quais dias cada
// funcionário atrasou (ou saiu cedo) e quanto tempo.
//
// Fonte da verdade: o MESMO motor da folha (computePeriodFolha → late_days),
// então o atraso aqui bate exatamente com a coluna ATRASOS da Folha do Mês.
...
import { CircleNotch as Loader2, Clock, Timer, Users, CheckCircle } from '@phosphor-icons/react';

const pad = (n: number) => String(n).padStart(2, '0');
const todayISO = () => { const d = new Date(); return new Date(d.getTime() - d.getTimezoneOffset() * 60000).toISOString().slice(0, 10); };
const fmtDia = (iso: string) => (iso ? iso.split('-').reverse().join('/') : '');
const dowShort = (iso: string) => ['dom', 'seg', 'ter', 'qua', 'qui', 'sex', 'sáb'][new Date(iso + 'T12:00:00').getDay()];
...
interface AtrasoRow { id: string; name: string; days: { date: string; minutes: number }[]; totalMin: number; }
...
export default function RelatorioAtrasos() {
  ...
  const { data: rows = [], isLoading, isFetching } = useQuery({
    ...
    queryFn: async (): Promise<AtrasoRow[]> => {
      ...
      for (const emp of (employees as any[]).filter(e => e.active)) {
        ...
        if (late.length > 0) out.push({ id: emp.id, name: emp.name, days: late, totalMin: late.reduce((s, d) => s + d.minutes, 0) });
      }
  ...
  return (
    <div className="space-y-4">
      ...
      <Panel eyebrow={`ATRASOS · ${fmtDia(from)}–${fmtDia(to)}`} title="Resumo por funcionário">
        {isLoading ? (
