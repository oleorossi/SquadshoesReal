...
const productionSectorsTab = read('src/components/technical-sheets/ProductionSectorsTab.tsx');
...
describe('salvamento da rota de produção da ficha técnica', () => {
  ...
  it('envia setores e etapas de Aviamento em uma única mutation', () => {
    const usage = section(technicalSheets, '<ProductionSectorsTab', '<Separator />');
    ...
    // Componente extraído na Fase 2 — contrato lê o arquivo dedicado.
    expect(productionSectorsTab).toContain('export function ProductionSectorsTab(');
    expect(productionSectorsTab).toContain('onSave(localSectors, localSteps)');
    expect(productionSectorsTab).toContain('disabled={saving}');
    expect(productionSectorsTab).not.toContain('onChangeAviamentoSteps');
  });
  ...
  it('salva capacidade por uma única mutation aguardada pelo componente pai', () => {
    ...
    const usage = section(technicalSheets, '<OperationsTab', '</TabsContent>');
  ...
  it('o save geral persiste somente o patch e não emite mutation para patch vazio', () => {
    const saveAll = section(technicalSheets, 'const saveAll = async () => {', 'const formatCurrency =');
