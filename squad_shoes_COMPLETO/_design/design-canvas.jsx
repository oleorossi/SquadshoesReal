  ...
  React.useLayoutEffect(() => {
    ...
    try {
      ...
      if (s && Number.isFinite(s.x) && Number.isFinite(s.y) && Number.isFinite(s.scale)) {
        // Sanity check: if saved position would put content way off-screen
        // (more than 2 viewports away in either direction), discard it.
        // Happens when canvas was panned aggressively and saved bad state.
        const vp = vpRef.current;
        const w = vp ? vp.clientWidth : 1200;
        const h = vp ? vp.clientHeight : 800;
        const offScreen = s.x > w * 1.5 || s.x < -w * 4 || s.y > h * 1.5 || s.y < -h * 4;
        if (!offScreen) {
