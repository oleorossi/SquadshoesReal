...
> **Histórico:** o projeto saiu de uma fase usando Lovable como editor + deploy. Em mai/2026 migrou pra Vercel + Codex. O pacote `lovable-tagger` ainda fica em `devDependencies` por compatibilidade com lockfile (não é mais usado em build — removido do `vite.config.ts`). Pra desinstalar de vez: rode `bun remove lovable-tagger` localmente e commite o novo lockfile.
...
|------|--------|------|--------------|
| `SessionStart` | `~/.Codex/session-start-sync.sh` | Start of session | Fetches `main`, **rebases** current branch on top. Falls back to merge `-X theirs` if rebase has conflicts. |
| `Stop` | `~/.Codex/stop-hook-git-check.sh` | End of turn | Faz commit + push da feature branch. Se a branch está ahead de main sem divergência, fast-forwarda main via push refspec (worktree-safe). Se diverge, faz merge local com `-X theirs`. |
...
|-----------|----------|-----------|
| Remote → Codex (session start) | `git rebase origin/main` | Commits remotos primeiro, Codex por cima — linear, sem overwrite. |
| Codex → main (session end) | rebase + `--ff-only` on main | Branch rebased fast-forwards main limpo. |
