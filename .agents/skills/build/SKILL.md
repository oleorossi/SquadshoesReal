...
Implement the requirements, honoring this repo's conventions (from AGENTS.md):
...
- **Design tokens, not hardcoded colors:** after visual edits run
  ...
  `bg-white`/`text-gray-*`. (Print/label components are exempt — see AGENTS.md.)
- **Canonical units** for any material/stock work (`m`, `dm²`, `un`, `par`,
  `placa`, `kg`, `L`) — respect the material-consumption rules in AGENTS.md.
