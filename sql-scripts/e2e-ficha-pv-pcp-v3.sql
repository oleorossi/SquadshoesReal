-- E2E v3 ficha(12 setores) → PV → estoque → PCP.
-- Escolhe cor com napa/receita pronta (CHAMPAGNE da v2 não tem SKU).
...
DECLARE
  ...
  v_gid uuid; v_diag jsonb; rec record; v_items jsonb;
BEGIN
  ...
  v_gid := resolve_strap_base_group_id(c_sp120, NULL);
  rep := rep || format('SP120 napa=%s napas_ativas=%s'||E'\n',
    (SELECT name FROM product_groups WHERE id = v_gid),
    (SELECT COALESCE(jsonb_agg(jsonb_build_object('c', p.color, 'sku', p.sku, 'q', p.quantity) ORDER BY p.color), '[]'::jsonb)
       FROM products p
  ...
  FOR rec IN
    ...
    ORDER BY CASE WHEN EXISTS (
      SELECT 1 FROM products p WHERE p.group_id = v_gid AND p.active AND p.unit = 'm' AND p.color = x.color
    ) THEN 0 ELSE 1 END, x.color
  LOOP
    BEGIN
      v_diag := diagnose_sale_order_internal_strap_readiness(c_sp120, NULL, rec.color);
    ...
    IF coalesce((v_diag->>'ready')::boolean, false) THEN
      ...
      rep := rep || format('OK 0 SP120 cor=%s'||E'\n', rec.color);
      EXIT;
    ELSE
      rep := rep || format('skip SP120 %s %s'||E'\n', rec.color, left(coalesce(v_diag->>'issues', v_diag::text), 140));
    END IF;
  END LOOP;
  ...
  v_gid := resolve_strap_base_group_id(c_i50, NULL);
  rep := rep || format('I50 napa=%s napas_ativas=%s'||E'\n',
    (SELECT name FROM product_groups WHERE id = v_gid),
    (SELECT COALESCE(jsonb_agg(jsonb_build_object('c', p.color, 'sku', p.sku, 'q', p.quantity) ORDER BY p.color), '[]'::jsonb)
       FROM products p
      WHERE p.group_id = v_gid AND p.active AND p.unit = 'm'
  ...
  FOR rec IN
    SELECT x.color FROM (
      ...
      UNION
      SELECT DISTINCT rcv.color FROM reference_color_variants rcv
       WHERE rcv.reference_id = c_i50 AND rcv.active AND coalesce(rcv.color,'') <> ''
    ) x
    ORDER BY CASE WHEN EXISTS (
      SELECT 1 FROM products p WHERE p.group_id = v_gid AND p.active AND p.unit = 'm' AND p.color = x.color
    ) THEN 0 ELSE 1 END, x.color
  LOOP
    BEGIN
      v_diag := diagnose_sale_order_internal_strap_readiness(c_i50, NULL, rec.color);
    EXCEPTION WHEN OTHERS THEN
      v_diag := jsonb_build_object('ready', false, 'issues', jsonb_build_array(jsonb_build_object('message', SQLERRM)));
    END;
    IF coalesce((v_diag->>'ready')::boolean, false) THEN
      v_color_i50 := rec.color;
      rep := rep || format('OK 0 I50 cor=%s'||E'\n', rec.color);
      EXIT;
    ELSE
      rep := rep || format('skip I50 %s %s'||E'\n', rec.color, left(coalesce(v_diag->>'issues', v_diag::text), 140));
    END IF;
  END LOOP;
  IF v_color_i50 IS NULL THEN
    rep := rep || 'WARN 0 I50 sem cor pronta — PV só com SP120'||E'\n';
  END IF;
