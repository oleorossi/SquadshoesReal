-- E2E v5 ficha(12 setores) → PV → estoque → PCP → faturamento (informal + formal).
-- Cobre Path B (nfe_required=false → Finalizado s/ NF) e Path A
-- (nfe_required=true → NF autorizada → Faturado → romaneio → Expedido).
...
DECLARE
  ...
  c_company uuid := '4dd13e86-5680-49e4-9943-5778f4f8df3d';
  ...
  -- faturamento
  v_so_formal uuid; v_op_formal uuid; v_ver bigint; v_ship int;
  v_cmd jsonb; v_nfe_id uuid; v_reserved_left int; v_mov_n int;
BEGIN
  ...
  -- ── 7. Informal: após 12 setores a OP deve estar Finalizado + settle ───────
  IF EXISTS (SELECT 1 FROM orders WHERE id=v_op_sp AND status='Finalizado') THEN
    rep := rep || 'OK 7 OP informal Finalizado após Expedição'||E'\n';
  ELSE
    fails := fails+1; rep := rep || format('FAIL 7 status OP informal=%s'||E'\n',
      (SELECT status FROM orders WHERE id=v_op_sp));
  END IF;

  SELECT count(*) INTO v_reserved_left
    FROM material_reservations
   WHERE order_id=v_op_sp AND status='reserved';
  IF v_reserved_left = 0 THEN
    rep := rep || 'OK 7b settle: zero reservas reserved após Finalizado'||E'\n';
  ELSE
    fails := fails+1; rep := rep || format('FAIL 7b ainda há %s reservas reserved'||E'\n', v_reserved_left);
  END IF;

  SELECT count(*) INTO v_mov_n FROM stock_movements WHERE order_id=v_op_sp;
  IF v_mov_n > 0 THEN
    rep := rep || format('OK 7c stock_movements=%s após settle'||E'\n', v_mov_n);
  ELSE
    -- tolerante: settle pode ter zero se não havia saldo (pending_reconciliation)
    SELECT count(*) INTO v_c FROM material_reservations
     WHERE order_id=v_op_sp AND status IN ('consumed','converted','pending_reconciliation');
    IF v_c > 0 THEN
      rep := rep || format('OK 7c settle sem on-hand (reservas terminal=%s, mov=0)'||E'\n', v_c);
    ELSE
      fails := fails+1; rep := rep || 'FAIL 7c sem movimentos e sem reservas terminais'||E'\n';
    END IF;
  END IF;

  -- Path B: romaneio informal Em Produção → Finalizado s/ NF
  BEGIN
    v_ship := register_order_shipment(ARRAY[v_so], NULL, 'e2e-v5-informal');
  EXCEPTION WHEN OTHERS THEN
    fails := fails+1; rep := rep || format('FAIL 7d register_order_shipment informal: %s'||E'\n', SQLERRM);
    v_ship := 0;
  END;
  IF v_ship >= 1 AND EXISTS (
       SELECT 1 FROM sale_orders WHERE id=v_so AND status='Finalizado s/ NF') THEN
    rep := rep || format('OK 7d Path B romaneio informal → Finalizado s/ NF (ship=%s)'||E'\n', v_ship);
  ELSE
    fails := fails+1; rep := rep || format(
      'FAIL 7d Path B ship=%s status=%s'||E'\n', v_ship,
      (SELECT status FROM sale_orders WHERE id=v_so));
  END IF;

  -- ── 8. Formal: NF autorizada → Faturado → romaneio → Expedido ─────────────
  v_items := jsonb_build_array(
    jsonb_build_object('reference_id', c_sp120, 'color', v_color_sp, 'quantity', v_qty,
      'unit_price', v_price, 'grade', v_grade_sp, 'fichas', 1, 'strap_colors', v_straps_sp,
      'material_variant_id', v_var_sp, 'observation', 'E2E formal')
  );
  BEGIN
    v_created := create_sale_order_atomic(
      jsonb_build_object('client_id', v_client, 'status', 'Rascunho',
        'notes', 'E2E formal rollback', 'packaging_mode', 'individual_amarrado',
        'nfe_required', true, 'nfe_external', false, 'order_type', 'carteira',
        'company_id', c_company, 'total', v_price*v_qty),
      v_items, gen_random_uuid());
    v_so_formal := (v_created->>'order_id')::uuid;
  EXCEPTION WHEN OTHERS THEN
    fails := fails+1; rep := rep || format('FAIL 8 create formal: %s'||E'\n', SQLERRM);
  END;

  IF v_so_formal IS NOT NULL THEN
    -- garante flags fiscais mesmo se o create ignorar nfe_* no header
    UPDATE sale_orders
       SET nfe_required = true, nfe_external = false, company_id = COALESCE(company_id, c_company)
     WHERE id = v_so_formal;

    BEGIN
      PERFORM promote_sale_order_to_production(v_so_formal, 'Aprovado');
      PERFORM promote_sale_order_to_production(v_so_formal, 'Em Produção');
    EXCEPTION WHEN OTHERS THEN
      fails := fails+1; rep := rep || format('FAIL 8b promote formal: %s'||E'\n', SQLERRM);
    END;

    SELECT o.id INTO v_op_formal
      FROM orders o JOIN sale_order_items soi ON soi.id = o.sale_order_item_id
     WHERE o.sale_order_id = v_so_formal AND o.status NOT IN ('Cancelada','Rascunho')
     LIMIT 1;

    IF v_op_formal IS NULL THEN
      fails := fails+1; rep := rep || 'FAIL 8c OP formal ausente'||E'\n';
    ELSE
      rep := rep || format('OK 8c OP formal=%s'||E'\n', v_op_formal);
    END IF;

    -- NF-e autorizada sintética (não chama edge emit-nfe; testa o gate do romaneio)
    INSERT INTO nfe_emitidas (
      id, sale_order_id, ref_nfe, status, numero, serie, chave_acesso,
      valor_total, data_emissao, company_id, cnpj_emitente, protocolo
    ) VALUES (
      gen_random_uuid(), v_so_formal, 'e2e-v5-'||v_so_formal::text, 'autorizada',
      '99999', '1', '33260962406033000193550010000999991234567890',
      COALESCE(v_price,1)*v_qty, now(), c_company, '62406033000193', 'e2e-proto'
    ) RETURNING id INTO v_nfe_id;
    rep := rep || format('OK 8d NF sintética autorizada %s'||E'\n', v_nfe_id);

    SELECT order_version INTO v_ver FROM sale_orders WHERE id = v_so_formal;
    BEGIN
      v_cmd := execute_sale_order_command(
        v_so_formal, 'transition', v_ver,
        'e2e-v5-faturado-'||v_so_formal::text,
        jsonb_build_object('target_status', 'Faturado'),
        NULL);
      IF EXISTS (SELECT 1 FROM sale_orders WHERE id=v_so_formal AND status='Faturado') THEN
        rep := rep || 'OK 8e transition → Faturado'||E'\n';
      ELSE
        fails := fails+1; rep := rep || format('FAIL 8e status=%s cmd=%s'||E'\n',
          (SELECT status FROM sale_orders WHERE id=v_so_formal), left(v_cmd::text,200));
      END IF;
    EXCEPTION WHEN OTHERS THEN
      fails := fails+1; rep := rep || format('FAIL 8e transition Faturado: %s'||E'\n', SQLERRM);
    END;

    -- Path A: Faturado + NF autorizada → Expedido (fecha estágios abertos)
    BEGIN
      v_ship := register_order_shipment(ARRAY[v_so_formal], NULL, 'e2e-v5-formal');
    EXCEPTION WHEN OTHERS THEN
      fails := fails+1; rep := rep || format('FAIL 8f romaneio formal: %s'||E'\n', SQLERRM);
      v_ship := 0;
    END;

    IF v_ship >= 1 AND EXISTS (
         SELECT 1 FROM sale_orders WHERE id=v_so_formal AND status='Expedido') THEN
      rep := rep || format('OK 8f Path A romaneio formal → Expedido (ship=%s)'||E'\n', v_ship);
    ELSE
      fails := fails+1; rep := rep || format(
        'FAIL 8f Path A ship=%s status=%s'||E'\n', v_ship,
        (SELECT status FROM sale_orders WHERE id=v_so_formal));
    END IF;

    IF v_op_formal IS NOT NULL THEN
      IF EXISTS (SELECT 1 FROM orders WHERE id=v_op_formal AND status='Finalizado') THEN
        rep := rep || 'OK 8g romaneio formal finalizou a OP'||E'\n';
      ELSE
        fails := fails+1; rep := rep || format('FAIL 8g OP formal status=%s'||E'\n',
          (SELECT status FROM orders WHERE id=v_op_formal));
      END IF;

      SELECT count(*) INTO v_c FROM order_stages
       WHERE order_id=v_op_formal AND status <> 'concluido';
      IF v_c = 0 THEN
        rep := rep || 'OK 8h todos os estágios fechados pelo romaneio'||E'\n';
      ELSE
        fails := fails+1; rep := rep || format('FAIL 8h %s estágios ainda abertos'||E'\n', v_c);
      END IF;

      SELECT count(*) INTO v_reserved_left
        FROM material_reservations
       WHERE order_id=v_op_formal AND status='reserved';
      IF v_reserved_left = 0 THEN
        rep := rep || 'OK 8i settle formal: zero reserved'||E'\n';
      ELSE
        fails := fails+1; rep := rep || format('FAIL 8i formal ainda reserved=%s'||E'\n', v_reserved_left);
      END IF;
    END IF;
  END IF;

  IF fails=0 THEN rep := E'\n===== E2E v5 VERDE (12 setores + informal + formal) =====\n'||rep;
  ELSE rep := format(E'\n===== E2E v5 FALHOU (%s) =====\n', fails)||rep; END IF;
