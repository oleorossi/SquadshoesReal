...
SELECT stage,
       ...
       (SELECT jsonb_build_object(
                 'wave_id', s2.wave_id, 'code', s2.wave_code,
                 'week_start', s2.week_start, 'week_end', s2.week_end,
                 'progress_pct', s2.progress_pct, 'total_pairs', s2.total_pairs,
                 'capacity_per_day', s2.capacity_per_day, 'started_at', s2.started_at)
          FROM stages s2
