-- ============================================================================
-- P0.1 — calculate_order_consumption_by_grade: converter unidade dos
--         ITENS-PADRÃO DE SOLADO (sole_standard_items) → bug crítico da COLA
-- ============================================================================
-- Auditoria 2026-06-19: o bloco `sole_standard_items` (cola/químicos atrelados ao
-- SOLADO) emitia `required = consumo × pares` na unidade de ORIGEM (ex.: 'g') e o
-- downstream (reserva/débito/custeio by-grade) tratava como unidade de ESTOQUE
-- (kg) → 1000×. Ex.: COLA PVC 23,33 g/par virou 23,33 kg/par; reserva inflou pra
-- 103.865 kg contra 118,99 kg reais. convert_reservation_to_out debita o reservado
-- cru com LEAST(reserved, estoque) → zeraria o estoque físico de cola na 1ª
-- conversão de picking, SEM deixar quantity negativo (silencioso).
--
-- Todos os outros blocos da função (cabedal/forro/palmilha/fachete/BOM) JÁ
-- convertiam (÷dm2_per_unit). Só os std-items não. Fix: aplicar
-- convert_to_product_unit(qty, unidade_origem, unidade_produto) — a MESMA função
-- usada no custeio (calculate_order_cost_item). Quando incompatível (kind
-- diferente, ex.: 'un'→'kg'), mantém o valor cru + conversion_warning em vez de
-- gravar errado silenciosamente. Resto da função preservado byte-a-byte.
--
-- ⚠ Datada DEPOIS da última redefinição (20260721150000) e de todas as migrations
-- aplicadas via MCP nesta sessão, p/ o db push não reverter.
-- ============================================================================
