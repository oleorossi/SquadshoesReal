-- ════════════════════════════════════════════════════════════════════════════
-- Fix ALTO (auditoria 2026-06-11) — update_sale_order_atomic descartava 4 campos
-- ════════════════════════════════════════════════════════════════════════════
-- O create (useCreateSaleOrder) persiste via spread {...order}, mas o UPDATE do
-- RPC lista colunas explícitas no SET e NÃO incluía:
--   • brand                          (marca por PV — private label/OEM)
--   • informacoes_complementares_nf  (texto livre das info compl. da NF-e)
--   • nfe_external                   (flag: NF emitida por outra empresa)
--   • external_nfe_number            (número da NF externa)
-- Logo, editar esses campos no PV não persistia (o valor antigo permanecia).
-- Mesma classe do bug histórico de strap_colors. v_merged já vem populado via
-- jsonb_populate_record(v_so, p_header), então basta adicioná-los ao SET.
-- (O frontend agora carrega nfe_external/external_nfe_number na edição —
-- migration/commit anterior — então o save não reseta a flag.)
-- Idempotente (CREATE OR REPLACE). Demais lógica inalterada.
...
BEGIN
  ...
  UPDATE public.sale_orders SET
    ...
    -- Campos antes descartados ao editar (auditoria 2026-06-11):
