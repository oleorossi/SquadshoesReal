-- ════════════════════════════════════════════════════════════════════════
-- Corrige a mensagem QUEBRADA de "estoque insuficiente" em debit_strap_stock.
-- (Reportado 2026-06-18 no resync da OP-2026-00982.)
--
-- BUG: plpgsql RAISE não suporta precisão printf. O formato usava '%.4f', que
-- o RAISE lê como '%' (substitui o próximo arg) + literal '.4f'. Resultado:
--   "disponível 0.4f, necessário 441.6000000000000000.4f metros."
-- FIX: arredondar os números no argumento (round(x,4)) e usar '%' puro.
--
-- (O resync deixa de bater nesta exceção ao passar p_force_soft=true — fix no
-- frontend resyncOPs.ts; aqui só a mensagem, que vale p/ qualquer caller hard.)
--
-- Timestamp > 20260804120000 (última migration) p/ db push não reverter.
-- Função reproduzida fiel ao banco; mudou só o texto/args do RAISE de estoque.
-- ════════════════════════════════════════════════════════════════════════

CREATE OR REPLACE FUNCTION public.debit_strap_stock(
  p_strap_colors jsonb, p_order_quantity integer, p_order_id uuid DEFAULT NULL::uuid,
...
BEGIN
  ...
  LOOP
    ...
    IF v_current_qty < v_required THEN
      -- FIX: '%.4f' não funciona em RAISE (plpgsql só tem '%'); arredonda no arg.
      RAISE EXCEPTION
