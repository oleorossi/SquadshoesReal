-- ============================================================================
-- FIX (auditoria do motor de consumo — Unidade #3, 2026-06-29):
-- REGRESSÃO de `direct_components` no caminho GRADED.
--
-- O bloco `direct_components` (aviamentos por par: binóculo/fivela/dedinho — ~33
-- itens em 29 fichas) foi ADICIONADO ao calculate_order_consumption_by_grade em
-- 20260628181300 (Fase 1 da reconciliação, "opção B"). As reescritas POSTERIORES
-- da função o PERDERAM:
--   • 20260721150000 (fix fachete/BOM) declarou "Base: definição LIVE de 2026-06-09"
--     — snapshot ANTERIOR a 28/06 → não tinha o bloco → dropou silenciosamente.
--   • 20260811120000 (conversão de massa dos itens-padrão) foi baseada na de cima
--     e nunca o readicionou.
-- Logo a versão LIVE atual do by_grade (canônica p/ custeio/MRP/reserva/débito de
-- TODO PV com grade) NÃO consome direct_components — exatamente o bug que a
-- 20260628181300 fechou volta a valer. Já o motor TS (computeConsumptionForItems)
-- e o escalar calculate_order_consumption CONTINUAM lendo direct_components:
-- divergência display(modal/ficha) ↔ custeio/MRP.
--
-- ⚠ A guarda run_consumption_parity_tests() NÃO pegou isso: ela só faz
-- introspecção TEXTUAL de fachete/conversão/palmilha-pronta — não compara
-- numericamente escalar×by_grade nem checa direct_components. Por isso esta
-- migration TAMBÉM estende a guarda com o case `bygrade_inclui_direct_components`,
-- pra travar a regressão no PR/CI (lado TS já é travado por orderConsumption.test.ts).
--
-- Esta migration reaplica a função IDÊNTICA à 20260811120000 (preservando a
-- conversão de massa dos itens-padrão, fachete, palmilha pronta unificada,
-- Forração Palmilha, BOM convertido e componentes-extras) + reinsere o bloco
-- direct_components ANTES do loop de sheet_materials (flat por par, com guard
-- v_covered_product_ids — igual à 20260628181300 e ao escalar). Idempotente
-- (CREATE OR REPLACE). NÃO aplicada no banco — aplicar via MCP pós-merge.
--
-- ⚠ Timestamp 2026-08-30: DEPOIS da última redefinição (20260811120000) p/ o
-- `supabase db push` não reverter o fix. (O timestamp 20260628210100 sugerido na
-- pauta da unidade ficaria ANTES da 20260811120000 → seria revertido — por isso
-- foi descartado.)
-- ============================================================================
...
DECLARE
  ...
  -- P0.1: conversão de unidade dos itens-padrão de solado
...
BEGIN
  ...
  -- ── Itens-padrão de solado (cola/químicos atrelados ao solado) ──
  -- P0.1: converter da unidade de CONSUMO (ssic.unit, ex.: 'g') para a unidade de
  -- ESTOQUE do produto (ex.: 'kg') via convert_to_product_unit. Sem isso o
  -- downstream tratava g como kg (1000×). Incompatível (NULL) → mantém cru + aviso.
  ...
  -- ── Componentes diretos (direct_components) — RESTAURADO (2026-06-29) ──
  -- Aviamentos por par (binóculo/fivela/dedinho). O escalar e o motor TS já lêem
  -- direct_components; o by_grade perdeu o bloco nas reescritas 20260721150000 /
  -- 20260811120000 → custeio/MRP/reserva/débito de PV com grade subcontava os
  -- aviamentos. Reinserido ANTES do loop de sheet_materials (idêntico à
  -- 20260628181300 e ao escalar): flat por par, required = quantity × v_total_qty,
  -- guard v_covered_product_ids evita dobrar com BOM/itens-padrão/component_accessory.
...
-- ─────────────────────────────────────────────────────────────────────────────
-- Estende a guarda de paridade com o case `bygrade_inclui_direct_components`.
-- A versão anterior (20260722120000) só checava fachete/conversão/palmilha-pronta
-- — não tinha como pegar a regressão de direct_components acima. Reaplica a função
-- INTEIRA (CREATE OR REPLACE) preservando os 8 cases existentes + o novo (9 de
-- contrato + 2 de existência = 11). O wrapper vitest
-- (consumptionService.parity.test.ts) já exige >= 10.
-- ─────────────────────────────────────────────────────────────────────────────
...
BEGIN
  ...
  -- NOVO (2026-06-29): direct_components presente nos DOIS caminhos. O motor TS
  -- (computeConsumptionForItems) também lê direct_components — sem isto o by_grade
  -- (custeio/MRP/débito) subcontava aviamentos por par vs. o modal/ficha.
  case_name := 'bygrade_inclui_direct_components';
  ok := v_bygrade ILIKE '%direct_components%';
  message := 'by_grade deve ler direct_components (aviamentos por par), igual ao escalar e ao motor TS'; RETURN NEXT;
END;
$$;
...
COMMENT ON FUNCTION public.run_consumption_parity_tests() IS
  ...
  'dm²→unidade nos dois, fachete + direct_components no graded). Falha se uma '
  'versão divergente for deployada. Lado TS travado por orderConsumption.test.ts.';
