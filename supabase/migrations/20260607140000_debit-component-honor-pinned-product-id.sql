-- "Componentes Extras do Cabedal" (components_accessories) com ITEM fixado.
--
-- Pedido user 2026-06-07: ao adicionar componente, selecionar grupo E depois o
-- item, pra o débito acertar o produto exato. Quando o grupo tem variações que
-- NÃO são por cor (ex.: elástico 6mm vs 8mm), a resolução por grupo+cor escolhia
-- o produto errado.
--
-- O frontend passa a gravar `product_id` (+ product_name) na entrada do componente.
-- Aqui, no débito real de estoque, `product_id` tem PRIORIDADE: se presente,
-- debita o item exato; senão mantém o legado `id` (entradas de produto direto);
-- senão resolve por grupo+cor (comportamento padrão, preserva materiais que
-- variam por cor). Mudança aditiva e idempotente (CREATE OR REPLACE).
...
DECLARE
  ...
  v_product_id uuid;
BEGIN
  -- ========== 1) DEBIT sheet_materials (BOM) — validação ==========
  FOR mat IN
  ...
  -- ========== 1b) DEBIT sheet_materials (BOM) — débito ==========
  FOR mat IN
    SELECT sm.product_id, sm.quantity_per_unit, p.quantity AS current_stock, p.name, p.group_id, p.color AS product_color
    FROM public.sheet_materials sm
    JOIN public.products p ON p.id = sm.product_id
  ...
  IF v_components IS NOT NULL AND jsonb_typeof(v_components) = 'array' AND jsonb_array_length(v_components) > 0 THEN
    ...
    LOOP
      ...
      -- Item fixado pelo usuário (product_id) tem PRIORIDADE; senão o legado 'id'
      -- (entradas de produto direto); senão resolve por grupo+cor.
      v_product_id := NULL;
      BEGIN
      ...
      IF v_product_id IS NOT NULL THEN
        ...
        END IF;
      ELSE
        -- Material extra por grupo (resolve por cor)
