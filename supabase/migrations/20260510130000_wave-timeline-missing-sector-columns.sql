-- =============================================================================
-- Wave timeline: add missing sector start-date columns
-- =============================================================================
--
-- Migration 20260506120000_sector-rename-wave-stages.sql added
-- silk_start_date, colagem_start_date, solagem_start_date to the
-- compute_wave_timeline() return type but forgot to:
--   1. ALTER TABLE production_waves to add those columns.
--   2. Update update_wave_timeline() to persist them.
--
-- Without this fix, the three new start-dates are always NULL when a wave is
-- read back from the database (even though compute_wave_timeline() calculates
-- them correctly in memory for the preview step).
-- =============================================================================
...
DECLARE
  ...
  v_tl             record;
BEGIN
  -- Collect sale_order_ids linked to this wave
  ...
  -- Persist all timeline columns (including the three added in sector-rename migration)
