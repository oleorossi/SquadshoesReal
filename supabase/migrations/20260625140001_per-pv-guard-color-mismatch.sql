-- ════════════════════════════════════════════════════════════════════════════
-- GUARD: bloquear OC quando material cai em COR não cadastrada (fallback silencioso)
-- ════════════════════════════════════════════════════════════════════════════
-- Pedido do dono (2026-06-25): o sistema deve BLOQUEAR/alertar qualquer geração
-- que use uma cor que não existe cadastrada — o erro que fez a forração MARROM do
-- EC23 (PV-00141) cair em NAPA SUDANI CAPUCCINO (a SUDANI não tem MARROM) e o
-- consumo aparecer partido (7 em vez de 26).
--
-- calculate_order_consumption JÁ marca essas linhas com matched_by='color_mismatch'
-- (cor pedida sem produto → caiu noutra cor). Aqui compute_materials_per_pv passa a
-- expor a coluna color_mismatch (bool_or do matched_by por produto+cor resolvido).
-- A UI (GeneratePurchaseOrdersDialog) usa isso pra marcar a linha em vermelho e
-- BLOQUEAR "Confirmar e Gerar" até a cor ser cadastrada (com override consciente
-- pra casos benignos, ex.: base sem cor).
-- ════════════════════════════════════════════════════════════════════════════
