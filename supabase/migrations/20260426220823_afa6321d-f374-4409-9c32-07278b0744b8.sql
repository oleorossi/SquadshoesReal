-- 1. Campos de controle manual
ALTER TABLE public.sale_orders
  ADD COLUMN IF NOT EXISTS manual_billing_override boolean NOT NULL DEFAULT false,
  ADD COLUMN IF NOT EXISTS original_min_billing_date date,
  ADD COLUMN IF NOT EXISTS manual_override_reason text;

COMMENT ON COLUMN public.sale_orders.manual_billing_override IS
  'TRUE quando o usuário definiu delivery_deadline abaixo da data mínima calculada';
COMMENT ON COLUMN public.sale_orders.original_min_billing_date IS
  'Data mínima calculada que estava vigente quando o override manual foi feito';
COMMENT ON COLUMN public.sale_orders.manual_override_reason IS
...
BEGIN
  -- Para cada OP do pedido, pega o máximo lead time de cada setor
  SELECT
    COALESCE(MAX(
      CASE
  ...
  -- Maior lead time de fornecedor entre todos os materiais necessários
