-- =============================================================================
-- Fix update_sale_order_atomic — restores correct body from 20260510180000
-- =============================================================================
-- Migration 20260519130000 introduced a near-rewrite with four bugs:
--   (a) 8 columns silently dropped from the UPDATE (client_contact, representative,
--       delivery_week, delivery_month, notes, nfe, remessa, billing_week)
--   (b) Referenced non-existent sale_orders.economic_group_id → every PV edit fails
--   (c) Attempted to cast delivery_week to integer; column is text → crash
--   (d) Dropped NULLIF guard on delivery_deadline → empty-string crashes date cast
-- This migration reverts to the correct body from 20260510180000 and adds
