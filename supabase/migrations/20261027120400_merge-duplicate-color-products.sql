-- Fusão de produto duplicado por erro de cor (CAPUCCINO × CAPPUCCINO)
-- ============================================================================
-- O grupo NAPA SOFT tem CAPUCCINO e CAPPUCCINO, e COGUMELO em dois produtos;
-- DUBLAGEM tem 2 cores duplicadas. Duplicata de cor faz `resolve_material_product`
-- escolher por `ORDER BY quantity DESC LIMIT 1` — ou seja, o débito baixa de um
-- rolo e a compra repõe o outro.
--
-- Não existia como fundir. Esta função move o estoque, repõe as referências de
-- CADASTRO no sobrevivente e desativa o duplicado.
--
-- ⚠ O que NÃO migra, de propósito: histórico e documentos.
--   • `stock_movements`, `inventory_transactions`, `product_price_log`,
--     `material_audit_log` etc. são fatos sobre AQUELE produto.
--   • `purchase_order_items`, `invoice_items`, `sale_order_items`,
--     `picking_items` são documentos emitidos — reescrever falsearia o passado.
--   • `material_reservations` migra só o que ainda está ATIVO (reserved /
--     partially_consumed): a OP vai consumir do sobrevivente. Consumido é history.
--
-- ⚠ Guard: depois de repontar a whitelist, a função varre TODAS as FKs de
-- products e RAISE se ainda houver referência de cadastro pro duplicado que ela
-- não conhece. Falhar alto é melhor que deixar ficha apontando pra produto morto.
