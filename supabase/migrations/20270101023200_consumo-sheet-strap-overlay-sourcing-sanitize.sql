-- Consumo: overlay estrutural sem pins de catálogo da identidade antiga.
--
-- Sintoma residual (PV-00194 / NL03 TRASEIRA): a 231 já reidrata measure_id
-- para ELÁSTICO FORRADO 7 mm, mas o strap_sourcing ainda aponta variante/receita
-- de TIRA CHATA → preview marca origem congelada stale e o rótulo cai em
-- "TIRA 7 mm · NAPA SOFT" (measure_name = "7 mm" + prefixo TIRA no frontend).
--
-- Este patch, só no batch de Rascunho/Pendente com overlay:
--   1) remove strap_variant_id / recipe_id / finished_product_id do sourcing
--      das linhas marcadas consumo_sheet_structure_overlay
--   2) enriquece resolved.measure_name com "TIPO + display" (ELÁSTICO FORRADO 7 mm)
-- Marca: sheet_strap_overlay_sourcing_sanitize_232
...
COMMENT ON FUNCTION public.calculate_consumption_report_batch(uuid[], uuid[]) IS
  'Relatorio canonico: setor + ficha nomeada; preview une tiras da ficha ausentes do snapshot; em Rascunho/Pendente alinha identidade estrutural (medida/tipo) à ficha e descarta pins de catalogo da identidade antiga.';
...
BEGIN
  ...
  IF position('sheet_strap_overlay_sourcing_sanitize_232' IN v_batch) = 0 THEN
    RAISE EXCEPTION 'Guard: marker sheet_strap_overlay_sourcing_sanitize_232 ausente no batch';
  END IF;
