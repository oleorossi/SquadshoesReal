-- =============================================================================
-- PALMILHA — Fase 2: cálculo separado de PLACA (EVA) + FORRAÇÃO (napa do forro)
-- =============================================================================
-- Modelo (decisão do usuário 2026-05-31):
--   • Palmilha = PLACA (base EVA, consumo menor) + FORRAÇÃO (napa que cobre,
--     consumo maior, vem da napa do FORRO = lining_material). OU comprada PRONTA.
--   • PRONTA (technical_sheets.insole_ready_made OU sole.sole_classification =
--     'palmilha_pronta') → NÃO debita nada (nem placa, nem forração).
--   • A área por número vem do SOLADO: placa em sole_technical_specs.insole_consumption_dm2
--     (já existia, ramo "Palmilha"), forração em sole_technical_specs.insole_lining_consumption_dm2
--     (Fase 1) → herdada pela ficha em insole_lining_consumption_per_size.
--
-- Mudanças cirúrgicas nas duas funções de consumo (mesma lógica em ambas):
--   (1) v_is_palmilha_pronta = insole_ready_made OR sole.sole_classification='palmilha_pronta'.
--   (2) Ramo PLACA ("Palmilha") ganha guarda `AND NOT v_is_palmilha_pronta`.
--   (3) NOVO ramo "Forração Palmilha": só quando NOT pronta, insole_has_lining=true,
--       lining_material setado e área de forração > 0. Resolve a MESMA napa do forro
--       (lining_material, cor do cabedal), converte dm²→unidade física pela largura da
--       ficha (get_material_conversion_info) e debita como linha ADICIONAL ao "Forro".
--       O débito (hybrid_debit_stock_for_order) processa 1 reserva/linha sem dedup por
--       product_id → forro + forração somam naturalmente no mesmo produto.
--
-- Custeio (calculate_order_cost_item) e MRP herdam: ambos consomem o resultado destas
-- funções (snapshot via freeze_technical_sheet). Rollout seguro: enquanto a área de
-- forração estiver vazia (Fase 3 pendente), o ramo novo emite 0 linhas.
-- =============================================================================

-- ─── 1. calculate_order_consumption (caminho single-size) ───────────────────
...
BEGIN
  ...
  -- Palmilha pronta = não debita placa nem forração (decisão do usuário)
  ...
  -- que não existe em sole_technical_specs. Só lining e insole têm
  -- fallback via sole_technical_specs (cabedal não — depende exclusiva-
  -- mente de upper_consumption_per_size JSONB ou upper_consumption escalar).
  ...
  -- PLACA da palmilha (base EVA). Pulada quando palmilha é pronta.
  ...
  -- FORRAÇÃO da palmilha (napa do forro que cobre a placa). Linha ADICIONAL ao Forro,
  -- mesmo produto (lining_material, cor do cabedal). Só quando NOT pronta + tem forro.
...
-- ─── 2. calculate_order_consumption_by_grade (caminho primário do PV) ────────
...
BEGIN
  ...
  FROM jsonb_each_text(p_grade) WHERE key ~ '^[0-9]+$' AND (value)::numeric > 0;
  ...
  LOOP
    ...
    IF (v_upper IS NULL OR v_lining IS NULL OR v_insole IS NULL OR v_insole_lining IS NULL)
      ...
      IF FOUND THEN
        -- Cabedal NÃO tem fallback via sole_technical_specs (não existe upper_consumption_dm2 lá);
        -- depende exclusivamente de upper_consumption_per_size JSONB / upper_consumption escalar.
        -- (Mesma correção do caminho single-size feita em 22/05/2026; a versão by_grade tinha
        --  ficado com a referência morta v_spec.upper_consumption_dm2 — só não estourava porque o
        --  gate raramente disparava. A Fase 2 da palmilha ampliou o gate p/ insole_lining e expôs.)
    ...
    -- Overrides de variante só quando há variante (senão v_variant fica RECORD não
    -- atribuído → "record is not assigned yet"). A versão single-size já guardava isto;
    -- a by_grade acessava fora da guarda — bug pré-existente exposto ao reativar o gate.
    IF p_material_variant_id IS NOT NULL THEN
    ...
    END IF;
    -- Acumula forração da palmilha (mesma napa do forro). Só quando NOT pronta + tem forro.
  ...
  -- FORRAÇÃO da palmilha (napa do forro que cobre a placa). Linha ADICIONAL ao Forro,
  -- mesmo produto (v_lining_pid). Só quando NOT pronta + tem forro + área > 0.
  IF NOT v_is_palmilha_pronta
