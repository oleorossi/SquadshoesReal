...
--   (e em seguida, se passar, "PV possui N linha(s) de tira bloqueada(s)")
--
-- Causa: 20270101020400 reescreveu enqueue copiando o MUTEX pré-14675:
--   ficha com upper_material → compara esperado [] contra item.strap_colors
--   preenchido → DISTINCT FROM → RAISE falso.
-- A doutrina 14675/16100 é coexistência: cabedal e tiras são independentes.
--
-- Esta migration:
--   1) restaura o bloco de freeze da 16100 (coexist + color/material policy +
--      production_excluded_at)
--   2) preserva os no-ops de pré-baseline da 20400 (schedule_changed /
            ...
            SELECT coalesce(jsonb_agg(jsonb_build_object(
              ...
              FROM jsonb_array_elements(
                CASE
                  ...
                  -- (restaurado apos regressao da 20400)
...
COMMENT ON FUNCTION public.enqueue_sale_order_strap_demands(uuid, text, uuid) IS
  'Enfileira demanda de tiras do PV. Cabedal+tiras coexistem (14675). schedule_changed/item_updated sem baseline sao no-op (20400).';

-- Guard embutido: corpo vivo nao pode voltar ao MUTEX da 20400.
DO $$
DECLARE
  v_definition text;
BEGIN
