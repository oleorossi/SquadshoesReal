...
COMMENT ON FUNCTION public.enqueue_sale_order_strap_demands(uuid, text, uuid) IS
  'Enfileira demanda de tiras do PV. Cabedal+tiras coexistem (14675). Preview operacional privado (15500). schedule_changed/item_updated sem baseline sao no-op (20400).';

-- Guard embutido: corpo vivo nao pode voltar ao MUTEX / preview publico da 20400.
DO $$
DECLARE
  v_definition text;
BEGIN
