-- =============================================================================
-- AUDIT ROUND 15 — Cálculo de custo de materiais alternativos
-- =============================================================================
-- BUG CRÍTICO em calculate_order_consumption_by_grade:
--
-- 1. "Forro (alternativa)" (lining_accessories) emitia linha SOMANDO o custo,
--    SEM aplicar get_material_conversion_info. Resultado: consumo cadastrado
--    em dm² (~4.56) virava metros lineares (4.56m/par × 12 pares = 54.72m,
--    R$ 1.143 só de forro!). 16 PVs com margem < -50%, todos os 25 PVs
--    ativos afetados.
--
-- 2. Frontend MaterialConsumptionDialog trata alternativas como OPÇÕES
--    (escolhe uma baseado em cor/match), mas a função SQL SOMAVA todas.
--    Inconsistência de semântica.
--
-- 3. components_accessories (cabedal alternativo) era IGNORADO pela função
--    (gap), mesmo quando marcado como mandatory:true (= material fixo
--    obrigatório, deveria somar).
--
-- Fix:
--   A. Forro (alternativa) só emite quando mandatory:true (= material adicional
--      obrigatório, não opção alternativa). Aplica get_material_conversion_info.
--   B. Cabedal mandatórios (components_accessories com mandatory:true) emitidos
--      como linha "Cabedal (mandatório)" — gap fechado.
--   C. Alternativas (mandatory:false ou sem flag) deixam de inflar custo.
--
-- Auto-cost-recalc (round 10) recalcula automaticamente todos os 25 PVs.
-- =============================================================================
...
BEGIN
  ...
  LOOP
    ...
    IF v_upper IS NOT NULL THEN v_upper_source := 'sheet_per_size'; END IF;

    v_lining := NULLIF(COALESCE((v_sheet.lining_consumption_per_size ->>(v_size::text))::numeric, 0), 0);
    IF v_lining IS NOT NULL THEN v_lining_source := 'sheet_per_size'; END IF;

    v_insole := NULLIF(COALESCE((v_sheet.insole_consumption_per_size ->>(v_size::text))::numeric, 0), 0);
    IF v_insole IS NOT NULL THEN v_insole_source := 'sheet_per_size'; END IF;
  ...
  -- Forros mandatórios (round 15: só somam quando mandatory:true)
  -- Com get_material_conversion_info aplicado pra evitar dm² → m direto.
  IF COALESCE(v_sheet.insole_has_lining, true) = true
    ...
    FOR v_item IN SELECT value FROM jsonb_array_elements(v_sheet.lining_accessories) AS value LOOP
      ...
      v_is_mandatory := COALESCE((v_item ->> 'mandatory')::boolean, false);
      IF v_group_name IS NULL OR v_group_name = '' OR v_consumption <= 0 THEN CONTINUE; END IF;
      -- Round 15: NÃO emite alternativas — frontend MaterialConsumptionDialog
      -- já trata como opções (escolhe uma baseado em cor). Apenas mandatórios
      -- somam (forro adicional obrigatório, ex: forração interna + externa).
      IF NOT v_is_mandatory THEN CONTINUE; END IF;
      SELECT * INTO v_resolved FROM resolve_material_product(v_group_name, p_color, 0, false);
      IF v_resolved.product_id IS NULL THEN CONTINUE; END IF;
      IF v_resolved.product_id = ANY(v_covered_product_ids) THEN CONTINUE; END IF;
      SELECT * INTO v_conv FROM get_material_conversion_info(v_resolved.product_id);
      -- consumption está em dm² (padrão da indústria); converte pra unit do produto.
      v_required := ((v_consumption * v_total_qty)
                    / NULLIF(v_conv.dm2_per_unit, 0))
                    * (1 + COALESCE(v_conv.waste_pct, 0) / 100);
      v_result := v_result || jsonb_build_object(
        'component', 'Forro (mandatório)', 'product_id', v_resolved.product_id,
        ...
        'stock_ok', v_resolved.available_qty >= v_required,
        'debit_mode', 'soft', 'source', 'lining_accessory_mandatory',
        'matched_by', v_resolved.matched_by, 'unit', v_conv.target_unit);
      v_covered_product_ids := array_append(v_covered_product_ids, v_resolved.product_id);
    END LOOP;
  END IF;

  -- Cabedais mandatórios (round 15: gap fechado — antes ignorados)
  IF v_sheet.components_accessories IS NOT NULL
     AND jsonb_typeof(v_sheet.components_accessories) = 'array' THEN
    FOR v_item IN SELECT value FROM jsonb_array_elements(v_sheet.components_accessories) AS value LOOP
      v_group_name  := v_item ->> 'material';
      v_consumption := COALESCE((v_item ->> 'consumption')::numeric, 0);
      v_is_mandatory := COALESCE((v_item ->> 'mandatory')::boolean, false);
      IF v_group_name IS NULL OR v_group_name = '' OR v_consumption <= 0 THEN CONTINUE; END IF;
      -- Só mandatórios (ex: material fixo de reforço sempre presente).
      IF NOT v_is_mandatory THEN CONTINUE; END IF;
      SELECT * INTO v_resolved FROM resolve_material_product(v_group_name, p_color, 0, false);
      IF v_resolved.product_id IS NULL THEN CONTINUE; END IF;
      IF v_resolved.product_id = ANY(v_covered_product_ids) THEN CONTINUE; END IF;
      ...
      v_required := ((v_consumption * v_total_qty)
                    ...
                    * (1 + COALESCE(v_conv.waste_pct, 0) / 100);
      v_result := v_result || jsonb_build_object(
        'component', 'Cabedal (mandatório)', 'product_id', v_resolved.product_id,
        'product_name', v_resolved.product_name, 'color', p_color,
        'consumption_per_unit', ROUND(v_required / NULLIF(v_total_qty, 0), 4),
        'required', v_required,
        'available', v_resolved.available_qty,
        'stock_ok', v_resolved.available_qty >= v_required,
        'debit_mode', 'soft', 'source', 'cabedal_mandatory',
        'matched_by', v_resolved.matched_by, 'unit', v_conv.target_unit);
      v_covered_product_ids := array_append(v_covered_product_ids, v_resolved.product_id);
    END LOOP;
  END IF;
...
-- Marcar todos os PVs ativos como dirty pra forçar reprocessamento.
