-- Refinamento da migration 20260517180000.
--
-- Descoberta na validação: technical_sheets.strap_colors guarda só TEMPLATE
-- da tira (group_id + consumption), mas com color="" (vazio). A cor REAL
-- escolhida pelo cliente fica em sale_order_items.strap_colors.
--
-- Então o RPC precisa receber as cores reais do caller; com strap_colors
-- só da ficha, o resolve de product_id falha (color norm = '').
--
-- Mudanças desta migration:
-- 1. Novo parâmetro p_strap_colors jsonb DEFAULT NULL na assinatura.
-- 2. Se p_strap_colors passado → usa direto (caso normal vindo do PV).
-- 3. Senão → fallback pra ficha.strap_colors com override de cor vazia
--    pra p_color (operador escolheu uma cor única, aplica em todas as tiras).
--
-- Sem isso, a checagem voltava 0 linhas de shortage de tira mesmo com
-- estoque zero — exatamente o sintoma que travou OS pra "Seu nego" em
-- PVs como PV-2026-00097 / PV-2026-00067.

DROP FUNCTION IF EXISTS public.check_stock_availability(uuid, integer, text, jsonb) CASCADE;
DROP FUNCTION IF EXISTS public.check_stock_availability(uuid, integer, text, jsonb, jsonb) CASCADE;
...
DECLARE
  ...
  v_fichas numeric;
BEGIN
  -- Bloco 1: componentes regulares (sheet_materials) — inalterado
  FOR mat IN
  ...
  ELSE
    ...
    -- Override de cor: substitui color vazia/null por p_color
    SELECT jsonb_agg(
      CASE
...
GRANT EXECUTE ON FUNCTION public.check_stock_availability(uuid, integer, text, jsonb, jsonb) TO authenticated;
