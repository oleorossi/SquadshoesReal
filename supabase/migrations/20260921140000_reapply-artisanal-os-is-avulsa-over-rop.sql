-- Re-aplica is_avulsa=true na OS artesanal auto-gerada, AGORA sobre a definição
-- F3-1 (unidade de compra) de auto_create_purchase_order da mig
-- 20260920120000_rop-purchase-unit-and-artisanal.sql.
--
-- Contexto: a mig 20260921130000 já tinha corrigido o orphan guard
-- (INSERT da OS artesanal precisa de is_avulsa=true, senão
-- tg_service_order_orphan_guard aborta o faturamento — bug PV-00145). Mas a
-- 20260920120000 (feita em paralelo, "branch artesanal inalterado") recriou
-- auto_create_purchase_order com o branch artesanal ANTIGO, revertendo o fix →
-- o erro "OS precisa de vínculo com um PV ou OP — ou ser marcada como avulsa"
-- voltou.
--
-- Esta migration é a definição AUTORITATIVA (timestamp > ambas): mantém TODO o
-- F3-1 (item de OC em unidade de compra, exclusão do canal per_pv) E restaura
-- is_avulsa=true no único INSERT em service_orders (branch artesanal).
    ...
    IF NEW.is_artisanal = true THEN
      ...
      ELSE
        ...
        VALUES (
          ...
          -- satisfaz o orphan guard (mig 20260904120000). Restaurado após a
          -- 20260920120000 tê-lo revertido sem querer.
          true
        );
      END IF;
