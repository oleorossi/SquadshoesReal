...
CREATE OR REPLACE FUNCTION public.hybrid_debit_stock_for_order(
  p_reference_id uuid, p_order_quantity integer, p_color text DEFAULT ''::text, p_order_id uuid DEFAULT NULL::uuid
...
BEGIN
  ...
  -- The logic: only debit the lining option whose group contains the selected color.
  -- Try main lining first, then each lining_accessory. Only ONE gets debited.
  v_lining_debited := false;

  -- Try main lining material
  IF NOT v_lining_debited AND v_lining_material IS NOT NULL AND v_lining_material <> '' AND COALESCE(v_lining_consumption, 0) > 0 THEN
    ...
    IF p_color <> '' THEN
      ...
      END IF;
    END IF;
    -- If color matched in this group, debit it
    IF target_product_id IS NOT NULL THEN
      IF target_qty < required THEN
  ...
  -- Try lining accessories (alternative options)
  ...
  -- Fallback: if no color match found, debit main lining with any available product
  IF NOT v_lining_debited AND v_lining_material IS NOT NULL AND v_lining_material <> '' AND COALESCE(v_lining_consumption, 0) > 0 THEN
    required := v_lining_consumption * p_order_quantity;
    SELECT p.id, p.name, p.quantity INTO target_product_id, target_name, target_qty
    FROM products p JOIN product_groups pg ON pg.id = p.group_id
