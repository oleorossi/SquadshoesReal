...
-- Combina o fix de unidade (cm→m, já aplicado) com o fix de cor estrito:
-- - Mantém detecção de per-size data e conversão correta cm→meters.
...
BEGIN
  ...
  LOOP
    ...
    -- 2. Fallback apenas para produto genérico (sem cor)
    ...
    -- 3. Não encontrou: erro claro indicando cor errada se houver
    IF v_product_id IS NULL THEN
