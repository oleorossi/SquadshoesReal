...
WITH packaging_prices AS (
    SELECT 
        ...
        pc.packaging_type,
        pc.pairs_per_box,
        COALESCE(p.unit_price, bt.unit_price, 0) as unit_price
    FROM public.packaging_configs pc
    LEFT JOIN public.products p ON p.id = pc.product_id
    LEFT JOIN public.box_types bt ON bt.id = pc.box_type_id
    WHERE pc.active = true
)
SELECT 
    ts.id as sheet_id,
    COALESCE(SUM(unit_price / NULLIF(pairs_per_box, 0)), 0) as packaging_cost_per_pair
FROM public.technical_sheets ts
LEFT JOIN packaging_prices pc ON 
    pc.sheet_id = ts.id OR 
    pc.sole_group_id = ts.sole_group_id OR
    pc.sole_product_id IN (SELECT sole_product_id FROM public.resolve_sole_color(ts.id, ''))
GROUP BY ts.id;
...
BEGIN
  ...
  -- Custo de embalagem unitário da view
  SELECT packaging_cost_per_pair INTO v_packaging_unit
    FROM public.v_packaging_costs
   WHERE sheet_id = v_ref;
  
  v_packaging_total := COALESCE(v_packaging_unit, 0) * v_qty;
  ...
  -- O overhead incide sobre materiais e mão de obra (embalagem geralmente é custo direto fixo)
  v_overhead := v_overhead_pct * (v_material + v_labor);
  v_total := v_material + v_labor + v_packaging_total + v_overhead;
  ...
  v_out := jsonb_build_object(
    ...
    'labor_cost', v_labor,
    'packaging_cost', v_packaging_total,
    ...
    'breakdown', jsonb_build_object(
      ...
      'packaging_unit', v_packaging_unit,
