-- ════════════════════════════════════════════════════════════════════════════
-- FIX CRÍTICO: forração alternativa (lining_accessories) era ADITIVA no motor SQL
-- ════════════════════════════════════════════════════════════════════════════
-- Auditoria 2026-07-07 (PV-00146 / ficha DS21): o motor SQL
-- `calculate_order_consumption_by_grade` emitia a forração PRINCIPAL **mais** uma
-- linha "Forração (alternativa)" pra CADA item de `lining_accessories`, somando o
-- consumo como se o sapato usasse duas forrações inteiras. Ex.: PV-00146 pedia
-- PORCELANA → NAPA SOFT (forração principal, casa exato) + GLOW METALIC (alternativa,
-- cai em color_mismatch pois glow só tem CHAMPAGNE/PRATA). Resultado: consumo/custo/
-- compra/MRP inflados e aviso vermelho de "cor não cadastrada" espúrio.
--
-- `lining_accessories` são forrações ALTERNATIVAS (usa-se UMA — a principal quando
-- ela cobre a cor pedida, senão a 1ª alternativa que cobre), NÃO aditivas. O
-- frontend (SummaryConsumptionPanel.resolveOption / orderConsumption.ts) e o custeio
-- da margem ("only primary forração counts") já tratavam assim; só o motor SQL
-- divergia — e ele alimenta custeio (calculate_order_cost_item), compras
-- (compute_materials_per_pv), MRP (get_wave_material_needs), reservas
-- (try_reserve_materials) e débito (hybrid_debit_stock_for_order /
-- record_order_consumption). Este fix fecha a divergência num único ponto.
--
-- Comportamento novo (espelha resolveOption):
--   1) forração principal cobre a cor pedida (ou pedido sem cor) → usa a principal;
--   2) senão, 1ª alternativa (na ordem do array) cujo grupo cobre a cor → usa ela;
--   3) senão (ninguém cobre a cor) → mantém a principal, que resolve em
--      color_mismatch e faz o guard de "cor não cadastrada" aparecer (comportamento
--      desejado: sinalizar, não substituir cor em silêncio).
-- Nenhuma linha "Forração (alternativa)" é mais emitida.
-- ════════════════════════════════════════════════════════════════════════════

-- "O grupo cobre a cor pedida?" = resolve_material_product não cai em color_mismatch
-- (exact_color / partial_name / group_generic para grupos color-agnostic). DRY com o
-- resolvedor canônico pra não divergir da regra de casamento de cor.
CREATE OR REPLACE FUNCTION public.group_covers_color(p_group_name text, p_color text)
 RETURNS boolean
 LANGUAGE sql
 STABLE
 SECURITY DEFINER
...
AS $function$
  SELECT COALESCE(
    (SELECT r.product_id IS NOT NULL AND r.matched_by <> 'color_mismatch'
       FROM public.resolve_material_product(p_group_name, p_color, 0, false) r),
    false);
$function$;
...
DECLARE
  ...
  -- Forração pick-one (nunca aditivo): grupo/consumo da opção escolhida.
  v_lining_group text;
...
BEGIN
  ...
  -- ══ Forração: escolhe UMA opção (principal OU 1 alternativa), NUNCA aditivo ══
  -- Espelha resolveOption do frontend. lining_accessories = forrações ALTERNATIVAS.
  IF v_sheet.lining_material IS NOT NULL AND v_sheet.lining_material <> ''
    ...
    IF p_color IS NULL OR btrim(p_color) = ''
      ...
      v_lining_group := v_sheet.lining_material;                       -- (1) principal cobre
    ELSIF jsonb_typeof(v_sheet.lining_accessories) = 'array' THEN
      SELECT t.acc->>'material', (t.acc->>'consumption')::numeric      -- (2) 1ª alt que cobre
      ...
      ELSE
        v_lining_group := v_sheet.lining_material;                     -- (3) ninguém cobre → principal (mismatch surge o guard)
      END IF;
    ELSE
  ...
  ELSIF COALESCE(v_sheet.insole_has_lining, true) = true
    ...
    -- Sem forração principal, mas há alternativas: pick-one (prioriza a que cobre a cor)
  ...
  LOOP
    ...
    -- Forração alternativa usa o consumo flat da PRÓPRIA alternativa (sem per-size/sole).
  ...
  -- NOTA: a seção "Forração (alternativa)" (loop aditivo sobre lining_accessories)
  -- foi REMOVIDA — agora a forração é resolvida pick-one lá em cima (uma opção só).
