-- =============================================================================
-- Versiona a RPC debit_packaging_for_order que hoje vive só no banco (foi
-- aplicada via MCP sem migration). Sem este arquivo, qualquer `supabase db push`
-- futuro recriaria a versão antiga (sem p_force_soft) e quebraria os call sites.
--
-- Também muda o tratamento de packaging_mode = 'individual_amarrado' (legado):
-- agora é sinônimo de 'individual_fitilho' (debita individual + fitilho). Antes
-- caía no ELSE e debitava só individual — fitilho nunca saía do estoque, mesmo
-- quando o PV foi criado explicitamente como amarrado.
-- =============================================================================

DROP FUNCTION IF EXISTS public.debit_packaging_for_order(uuid, uuid, uuid, integer, text) CASCADE;
...
BEGIN
  ...
  ELSIF p_packaging_mode IN ('individual_fitilho', 'individual_amarrado') THEN
    -- 'individual_amarrado' é o nome legado do modo amarrado. Sinônimo de
    -- 'individual_fitilho': debita caixa individual + fitilho (metros).
  ...
  -- Caminho primário: product_groups (UI atual)
  IF v_sole_group_id IS NOT NULL THEN
    ...
    LOOP
      ...
      -- SOFT mode: apenas projeta, não toca estoque (usado em re-reserva de PV
      -- voltando pra Rascunho — débito real só nos caminhos hard).
  ...
  -- Fallback: packaging_configs (legacy, mantido por compatibilidade)
  IF NOT v_handled AND NOT p_force_soft THEN
    DECLARE
      cfg RECORD;
    BEGIN
      FOR cfg IN
...
COMMENT ON FUNCTION public.debit_packaging_for_order(uuid, uuid, uuid, integer, text, boolean) IS
  'Debita embalagem da OP. Lê primeiro de product_groups (modelo atual); fallback em packaging_configs (legacy). Modo soft (p_force_soft=true) apenas projeta sem mexer no estoque. individual_amarrado é sinônimo de individual_fitilho.';
