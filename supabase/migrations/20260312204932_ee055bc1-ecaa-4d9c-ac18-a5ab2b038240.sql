...
BEGIN
  ...
  IF v_components IS NOT NULL AND jsonb_typeof(v_components) = 'array' AND jsonb_array_length(v_components) > 0 THEN
    ...
    LOOP
      ...
      IF v_group_name IS NULL OR v_group_name = '' OR v_consumption <= 0 THEN
        CONTINUE;
      END IF;
      ...
      IF target_qty < required THEN
        RAISE EXCEPTION 'Estoque insuficiente para material extra "%" (%): disponivel %, necessario %',
          target_name, COALESCE(p_color, ''), target_qty, required;
      END IF;
      ...
      VALUES (target_product_id, 'out', required, target_qty, target_qty - required,
              'Debito Extra (' || v_group_name || ')' || CASE WHEN p_color <> '' THEN ' Cor: ' || p_color ELSE '' END);
    END LOOP;
  END IF;
END;
$function$
