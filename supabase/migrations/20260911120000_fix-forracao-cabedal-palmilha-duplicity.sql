-- Anti-duplicidade FORRAÇÃO (cabedal × palmilha) — motor de consumo canônico.
--
-- PROBLEMA (auditoria PV-00146, 2026-07-10):
--   Fichas de sandália (DS/EC/I/S/SP/ST/TR — 26 no total) emitiam DUAS linhas de
--   forração a partir da MESMA napa:
--     • "Forração" (cabedal)        ← ficha  technical_sheets.lining_consumption (escalar)
--     • "Forração Palmilha"         ← solado sole_technical_specs.insole_lining_consumption_dm2
--   Ambas resolvem pro mesmo produto/cor. O forro dessas sandálias é SÓ o da
--   palmilha (o "cabedal" são tiras; cabedal não leva forro — quando o cabedal
--   consome material, é o material do campo Cabedal / upper_material). O valor do
--   escalar (~5,7 dm²/par) é a própria área da palmilha, digitada por engano no
--   campo de forro do cabedal → a napa era contada 2× em consumo, compras, MRP,
--   ondas e CUSTEIO/margem.
--
-- CAUSA no motor: no loop por numeração, o forro de cabedal (v_lining) faz
--   fallback pro escalar da ficha (`COALESCE(v_lining, v_sheet.lining_consumption, 0)`)
--   quando o solado não tem forro de cabedal próprio (lining_consumption_dm2 nulo).
--
-- FIX (régua no motor, prevenção de recorrência): quando o solado DIRIGE o forro
--   de PALMILHA (insole_lining_consumption_dm2 > 0 em alguma numeração) e NÃO tem
--   forro de cabedal (lining_consumption_dm2 nulo em todas), o lining_material da
--   ficha É o forro da palmilha — não existe forro de cabedal. Zera v_lining pra
--   NÃO emitir a "Forração" fantasma. v_lining_pid (usado pela Forração Palmilha),
--   a "Palmilha" (placa) e a "Forração Palmilha" seguem intactas.
--
-- ESCOPO / SEGURANÇA: só dispara com sole_drives_consumption=true + solado com
--   forro-de-palmilha + solado SEM forro-de-cabedal. As 9 fichas que têm
--   lining_consumption>0 mas solado SEM forro de palmilha (fechados: STX, ST702…)
--   NÃO disparam a régua → forro delas preservado. Nenhuma ficha no banco popula
--   sole_technical_specs.lining_consumption_dm2 (forro de cabedal via solado), que
--   permanece o caminho canônico caso um dia exista forro de cabedal real.
--
-- calculate_order_consumption (escalar) é wrapper de _by_grade → herda o fix.
-- Consumidores (compute_materials_per_pv, calculate_order_cost, try_reserve/
-- débito, MRP, ondas) passam por esta função → todos corrigidos de uma vez.
...
DECLARE
  ...
  -- Anti-duplicidade forração (cabedal × palmilha):
...
BEGIN
  ...
  -- Anti-duplicidade forração: solado dirige forro de PALMILHA e NÃO tem forro de
  -- cabedal → o lining_material da ficha é o forro da palmilha; não emitir
  -- "Forração" (cabedal) fantasma. Ver cabeçalho da migration.
  ...
  IF v_sheet.lining_material IS NOT NULL AND v_sheet.lining_material <> ''
    ...
    END IF;
  ELSIF COALESCE(v_sheet.insole_has_lining, true) = true
  ...
  LOOP
    ...
    -- Anti-duplicidade: zera o forro de cabedal quando ele é, na verdade, o forro
    -- da palmilha (dirigido pelo solado). Aplica-se DEPOIS de todas as fontes
    -- (per_size / solado / escalar / variante / alternativa) pra não escapar por
    -- nenhum caminho. A Forração Palmilha (v_insole_lining) fica intacta.
  ...
  IF v_lining_pid IS NOT NULL AND COALESCE((v_acc_lining->>'required')::numeric, 0) > 0 THEN
    ...
    v_result := v_result || jsonb_build_object(
      ...
      'source', CASE WHEN v_lining_is_alt THEN 'lining_alt'
                     WHEN v_resolved.matched_by = 'variant' THEN 'variant' ELSE 'sheet_per_size' END,
      'matched_by', v_resolved.matched_by, 'unit', v_conv.target_unit,
      'conversion_warning', v_conv.conversion_warning);
...
-- Guard em /diagnostics: sinaliza as fichas onde o forro de cabedal
-- (lining_consumption) é a MESMA napa do forro de palmilha (solado dirige
-- insole_lining_consumption_dm2, sem lining_consumption_dm2). O motor já SUPRIME
-- a "Forração" fantasma dessas fichas; esta linha marca o lining_consumption como
-- DADO MORTO pra limpeza (ou, se houver forro de cabedal real, cadastrar
-- sole_technical_specs.lining_consumption_dm2 no solado). Severidade 'baixo'.
...
BEGIN
  ...
  -- M2a: per-size MUITO divergente do escalar (>3× ou <1/3) — pega typo tipo forro 0,5 vs 5,7 (A1).
  RETURN QUERY
  WITH persize AS (
  ...
  -- M2b: consumo implausivelmente ALTO (>100 por par) de cabedal/forro/palmilha.
  RETURN QUERY
  ...
  -- Anti-duplicidade forração (auditoria 2026-07-10): lining_consumption é a MESMA
  -- napa do forro de palmilha (solado dirige insole_lining, sem cabedal). O motor
  -- já suprime a "Forração" fantasma; sinaliza o campo como dado morto pra limpeza.
  RETURN QUERY
