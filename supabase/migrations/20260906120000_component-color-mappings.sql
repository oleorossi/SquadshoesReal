-- Componentes por Cor (opt-in por ficha) — modelo DS05 e afins.
--
-- Problema: os componentes avulsos da ficha (technical_sheets.direct_components,
-- JSONB debitado por product_id FIXO) são os mesmos pra qualquer cor do pedido.
-- Alguns modelos precisam de componentes DIFERENTES por cor predominante do item
-- (ex.: DS05 Caramelo = ABS Turqueza + ABS Marrom; Off White = Redondo Pérola + ABS
-- Marrom). O componente NÃO casa por nome de cor com a cor do item, então é um
-- mapeamento curado (cor → lista de SKUs), não resolução automática.
--
-- Recurso OPT-IN: só age quando technical_sheets.component_colors_enabled = true.
-- Ficha com a flag desligada (o padrão) = comportamento idêntico ao de hoje.

-- 1) Flag opt-in por ficha.
ALTER TABLE public.technical_sheets
  ADD COLUMN IF NOT EXISTS component_colors_enabled boolean NOT NULL DEFAULT false;

COMMENT ON COLUMN public.technical_sheets.component_colors_enabled IS
  'Opt-in: quando true, os componentes da ficha vêm de technical_sheet_component_colors '
  '(por cor predominante do item) em vez de direct_components. Poucos modelos usam.';

-- 2) Tabela de mapeamento cor predominante → lista de componentes (mini-BOM por cor).
CREATE TABLE IF NOT EXISTS public.technical_sheet_component_colors (
  id uuid PRIMARY KEY DEFAULT gen_random_uuid(),
  sheet_id uuid NOT NULL REFERENCES public.technical_sheets(id) ON DELETE CASCADE,
  cabedal_color text NOT NULL,      -- cor predominante do item (= sale_order_items.color); nome herdado das tabelas irmãs (lining/palmilha)
  product_id uuid NOT NULL REFERENCES public.products(id),
  quantity_per_unit numeric NOT NULL DEFAULT 0,   -- qtd por par (unidade nativa do produto)
  created_at timestamptz NOT NULL DEFAULT now()
);

COMMENT ON TABLE public.technical_sheet_component_colors IS
  'Componentes por cor predominante (opt-in via technical_sheets.component_colors_enabled). '
  '1 cor → N componentes (cada cor lista TUDO). Substitui direct_components quando há linhas '
  'pra a cor do pedido; senão cai em direct_components (fallback).';

CREATE INDEX IF NOT EXISTS idx_component_color_sheet
  ON public.technical_sheet_component_colors(sheet_id);

-- Unicidade case-insensitive por (ficha, cor, produto). Sem unaccent (não é IMMUTABLE);
-- a cor vem sempre da mesma fonte (products.color do grupo predominante), então lower+btrim basta.
CREATE UNIQUE INDEX IF NOT EXISTS uq_component_color
  ON public.technical_sheet_component_colors(sheet_id, lower(btrim(cabedal_color)), product_id);

-- 3) RLS — espelha technical_sheet_lining_colors (is_approved_user() em todo CRUD).
ALTER TABLE public.technical_sheet_component_colors ENABLE ROW LEVEL SECURITY;

DROP POLICY IF EXISTS approved_select ON public.technical_sheet_component_colors;
DROP POLICY IF EXISTS approved_insert ON public.technical_sheet_component_colors;
DROP POLICY IF EXISTS approved_update ON public.technical_sheet_component_colors;
DROP POLICY IF EXISTS approved_delete ON public.technical_sheet_component_colors;

CREATE POLICY approved_select ON public.technical_sheet_component_colors
  FOR SELECT TO authenticated USING ((SELECT public.is_approved_user()));
CREATE POLICY approved_insert ON public.technical_sheet_component_colors
  FOR INSERT TO authenticated WITH CHECK ((SELECT public.is_approved_user()));
CREATE POLICY approved_update ON public.technical_sheet_component_colors
  FOR UPDATE TO authenticated USING ((SELECT public.is_approved_user())) WITH CHECK ((SELECT public.is_approved_user()));
CREATE POLICY approved_delete ON public.technical_sheet_component_colors
  FOR DELETE TO authenticated USING ((SELECT public.is_approved_user()));

-- 4) Consumo: injeta os componentes por cor na fonte única de consumo.
--    calculate_order_consumption é só um wrapper de _by_grade, então só esta muda.
--    hybrid_debit_stock_for_order já debita cada linha do snapshot por product_id —
--    linhas source='component_color' se comportam igual a 'direct_components'.
...
BEGIN
  ...
  -- ── Componentes por cor (opt-in: technical_sheets.component_colors_enabled) ──
  -- Quando a ficha liga o recurso E existe mapeamento pra a cor do pedido, ESSAS
  -- linhas são o conjunto de componentes do item (cada cor lista tudo) e o
  -- direct_components é IGNORADO. Sem a flag, ou sem linha pra a cor, cai no
  -- direct_components (fallback). Match de cor accent/case-insensitive
  -- (extensions.unaccent qualificado — search_path é 'public').
