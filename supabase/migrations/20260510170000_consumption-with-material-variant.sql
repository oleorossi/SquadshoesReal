-- =============================================================================
-- Consumo de materiais: propagar `material_variant_id` até as RPCs
-- =============================================================================
--
-- Quando uma referência tem variações de material (ex.: "Sandália 101" feita
-- em Couro / Napa / Sintético), cada variação tem um `upper_material_product_id`
-- opcional que aponta para um SKU específico de matéria-prima. Hoje o cálculo
-- de consumo SEMPRE resolvia pelo nome do grupo (`technical_sheets.upper_material`),
-- ignorando o override fiscal da variação. Para fichas que diferem apenas no
-- SKU/NCM da NF-e isso não tem impacto no estoque, mas se no futuro variações
-- precisarem de receitas distintas (ex.: cabedal de Sintético consome 5% menos
-- que Couro), o pipeline já está pronto.
--
-- Esta migration:
--   1) Cria `resolve_upper_material_for_variant()` — wrapper que respeita o
--      `upper_material_product_id` da variação quando definido, caindo no
--      `resolve_material_product()` padrão caso contrário.
--   2) Adiciona o parâmetro `p_material_variant_id uuid DEFAULT NULL` em
--      ambas `calculate_order_consumption` e `calculate_order_consumption_by_grade`,
--      passando-o ao novo wrapper na seção "Cabedal".
--
-- Comportamento sem variante (NULL): IDÊNTICO ao anterior.
-- =============================================================================
...
BEGIN
  IF p_variant_id IS NOT NULL THEN
    SELECT upper_material_product_id INTO v_pid
...
GRANT EXECUTE ON FUNCTION public.resolve_upper_material_for_variant(uuid, text, text, numeric) TO authenticated;

-- ── 2) calculate_order_consumption (single-size) ────────────────────────────
...
BEGIN
  ...
  -- Cabedal — agora respeita p_material_variant_id quando setado
...
-- ── 3) calculate_order_consumption_by_grade (graded) ────────────────────────
...
BEGIN
  ...
  -- Cabedal — respeita variação quando setada
  ...
  -- Cabedal — usa variação quando setada
...
COMMENT ON FUNCTION public.calculate_order_consumption(uuid, numeric, text, integer, uuid) IS
  'Single-size consumption. p_material_variant_id (NULL = ignorar) permite '
  'usar o `upper_material_product_id` da variação fiscal em vez de resolver '
  'pelo nome do grupo. Comportamento sem variante é idêntico ao anterior.';

COMMENT ON FUNCTION public.calculate_order_consumption_by_grade(uuid, jsonb, text, uuid) IS
  'Graded consumption. Mesmo override de variação que a função single-size.';
