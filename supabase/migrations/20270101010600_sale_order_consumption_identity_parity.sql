...
BEGIN
  IF p_variant_id IS NOT NULL THEN
    ...
    -- PV-CONSUMPTION-IDENTITY-PARITY: pin explícito sempre vence, inclusive
  ...
  -- PV-CONSUMPTION-IDENTITY-PARITY: area-first. `placa` fica fora de
  ...
  -- PV-CONSUMPTION-IDENTITY-PARITY: linear-only-fallback. Preserva a
...
DECLARE
  ...
  v_replacement text := $replacement$    -- PV-CONSUMPTION-IDENTITY-PARITY: embalagem-discreta-por-item.
...
BEGIN
  ...
  IF position('PV-CONSUMPTION-IDENTITY-PARITY: embalagem-discreta-por-item' IN v_definition) = 0 THEN
    v_occurrences := (
...
BEGIN
  ...
  ok := position('PV-CONSUMPTION-IDENTITY-PARITY: pin explícito' IN v_resolver) > 0
  ...
  ok := position('PV-CONSUMPTION-IDENTITY-PARITY: area-first' IN v_resolver) > 0
  ...
  ok := position('PV-CONSUMPTION-IDENTITY-PARITY: linear-only-fallback' IN v_resolver) > 0
  ...
  ok := position('PV-CONSUMPTION-IDENTITY-PARITY: embalagem-discreta-por-item' IN v_by_grade) > 0
