-- ============================================================================
-- A2 + B3 — try_reserve_materials: não inflar ~100× sem largura + dedup de OC
-- ============================================================================
-- Avaliação dos motores (2026-07-08). Duas correções ADITIVAS (o resto da
-- função é idêntico ao def vivo):
--
-- A2 (ALTO): material LINEAR sem `dimensions_width` na ficha de componente →
--   `get_material_conversion_info` devolve dm2_per_unit=1 + conversion_warning.
--   Os laços de BOM e de spec então deixavam a demanda em dm² CRU e mesmo assim
--   reservavam / geravam OC (~100× inflado). O modal TS neutraliza essa linha;
--   o SQL não. Fix: se `conversion_warning IS NOT NULL`, PULA o material (sem
--   reserva, sem OC) e registra nota — espelhando o comportamento neutro do
--   modal. `conversion_warning` só é setado nesse caso exato (verificado no def
--   vivo de get_material_conversion_info: é NULL p/ contagem/massa, p/ linear
--   COM largura e p/ tira/elástico sem ficha), então o guard não pula materiais
--   normais. A correção definitiva é cadastrar a largura (tarefa de cadastro).
--
-- B3 (MÉDIO): os ramos R3/R4/R5 criavam OC de reposição com INSERT cru, sem
--   checar se já existe OC ABERTA pro produto (ao contrário de
--   auto_create_purchase_order). Duas OPs curtas no mesmo material podiam pedir
--   a mesma falta 2×. Fix: antes de criar a OC, `IF EXISTS` OC aberta pro
--   produto → não duplica, só registra nota (mesmo guard do auto_create).
-- ============================================================================
...
BEGIN
  ...
  -- (a1) Gate canônico de palmilha pronta
  ...
  LOOP
    ...
    -- A2 (2026-07-08): largura faltando → não converte → não reserva/OC (senão ~100x).
    ...
    IF p_priority = 'urgent' AND v_available < v_demand THEN
      ...
      IF v_net_missing > 0 THEN
        -- B3: não duplica OC se já há uma aberta pro produto (mesmo guard do auto_create).
  ...
  FOREACH v_spec SLICE 1 IN ARRAY v_spec_materials LOOP
    ...
    -- (a1) palmilha pronta na cor: não reservar Palmilha
    ...
    SELECT * INTO v_conv4 FROM public.get_material_conversion_info(v_target_id);
    -- A2 (2026-07-08): largura faltando → não converte → não reserva (senão ~100x).
    IF v_conv4.conversion_warning IS NOT NULL THEN
      v_notes := v_notes || to_jsonb('IGNORADO (largura faltando, dm²→unidade impossível): ' || v_target_name);
      CONTINUE;
    END IF;
