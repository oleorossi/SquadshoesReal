-- ============================================================================
-- FIX CRÍTICO (auditoria 2026-06-09, itens C4 + C5):
--
-- C5 — Regressão: o componente "Fachete" (introduzido em 20260518120000) foi
--   perdido nas reescritas posteriores de calculate_order_consumption_by_grade
--   (20260607*, 20260629210000, 20260702190000). Como PV sempre tem grade,
--   custeio/snapshot/débito/MRP não reservavam nem debitavam o forro extra de
--   solados fachetados. Reintroduzido: acumulador per-size de
--   sole_technical_specs.fachete_lining_consumption_dm2 + linha 'Fachete'
--   (mesmo grupo de material do forro) + consumption_warning com tamanhos
--   sem valor.
--
-- C4 — Linha BOM (sheet_materials) emitia required = quantity_per_unit × qtd
--   CRU: sem conversão dm²→unidade física, sem perda%, sem campo 'unit' e com
--   available = quantity bruto. Efeitos: custo de napa via BOM ~100× inflado
--   (convert_to_product_unit com unit null retorna cru), reserva inflada e
--   stock_ok otimista. Agora: converte via get_material_conversion_info
--   (largura da ficha de componente) quando o produto é linear com largura
--   (dm2_per_unit ≠ 1), aplica perda%, emite 'unit' e deduz reserved_stock.
--   Compatível com fn_projected_demand (20260720120000): ele só divide linhas
--   com unit IS NULL — linha convertida agora carrega unit e passa direto.
--
-- Base: definição LIVE (pg_get_functiondef 2026-06-09), que já incluía
-- variantes, palmilha pronta, Forração Palmilha e Componente Extra.
-- ============================================================================
...
BEGIN
  ...
  -- C5: solado fachetado consome forro extra (fachete) por numeração
  ...
  LOOP
    ...
    -- Fallback per-size: sole_technical_specs. Entra quando
    -- sole_drives_consumption=true OU solado fachetado (fachete só mora lá).
    ...
    -- Fachete não tem média escalar na ficha — registra warning por tamanho.
    ...
    END IF;
    -- C5: fachete consome o MESMO grupo de material do forro
  ...
  -- C5: Fachete (mesmo grupo do forro, linha separada para auditoria)
  IF v_is_fachetado
    ...
    IF v_resolved.product_id IS NOT NULL THEN
      ...
      v_result := v_result || jsonb_build_object(
        ...
        'consumption_warning', CASE
          ...
          ELSE NULL
        END);
      -- Não incluir em covered_product_ids — o forro principal pode usar o
      -- mesmo product_id; o débito agrega por product_id.
    END IF;
  ELSIF v_is_fachetado AND array_length(v_warn_fachete_sizes, 1) > 0 THEN
    -- Fachetado sem NENHUM valor por tamanho: emitir aviso explícito
    v_result := v_result || jsonb_build_object(
  ...
  -- C4: BOM (sheet_materials) com conversão dm²→unidade física + perda% +
  -- unit + disponível líquido de reserva.
  FOR v_row IN
  ...
  LOOP
    ...
    IF COALESCE(v_conv.dm2_per_unit, 1) > 0 AND COALESCE(v_conv.dm2_per_unit, 1) <> 1 THEN
      -- Material de área cortado de bobina: quantity_per_unit está em dm²/par.
