...
-- F1-07: recompute_production_schedule ignorava feriados (agendava produção em
--         feriado obrigatório)
-- ════════════════════════════════════════════════════════════════════════════
-- O loop de alocação usava "IF EXTRACT(ISODOW FROM v_day) BETWEEN 1 AND 5" —
-- só pulava fim de semana. O motor de ondas (compute_wave_timeline via
-- add_business_days/is_business_day) e o TS (sectorCapacity.ts, cache de
-- holidays) pulam feriado com optional=false. Resultado: quando o horizonte do
-- schedule cruza um feriado (ex.: 2026-09-07, segunda), production_schedule
-- agenda pares num dia sem fábrica enquanto onda e UI pulam o dia — Gargalo
-- Diário mostra produção planejada num dia sem fábrica e o horizonte real
-- desliza 1 dia.
--
-- Fix (diff mínimo sobre a definição viva): o gate do loop passa a usar
-- public.is_business_day(v_day) — a MESMA fonte de feriados (tabela holidays,
-- optional=false) do motor de ondas. Todo o resto da função é idêntico.
