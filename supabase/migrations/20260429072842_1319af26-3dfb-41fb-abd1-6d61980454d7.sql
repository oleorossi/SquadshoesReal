...
BEGIN
  -- Verifica que a onda existe e está ativa
  ...
  -- Marca o estágio atual como completo
  ...
  -- Determina os próximos estágios a abrir conforme as regras
  v_next_stages := ARRAY[]::production_stage_enum[];

  IF p_stage = 'corte' THEN
    -- Corte termina → libera Palmilha e Costura imediatamente
    v_next_stages := ARRAY['palmilha','costura']::production_stage_enum[];

  ELSIF p_stage IN ('mesa','palmilha','costura') THEN
    -- Montagem só inicia quando os três (mesa, palmilha, costura) — entre os existentes na onda — estiverem completos
    IF NOT EXISTS (
  ...
  -- Abre os próximos estágios (apenas os que existem na onda e estão pendentes)
  v_opened := NULL;
  ...
  -- Atualiza current_stage para um dos abertos (informativo)
...
BEGIN
  ...
  IF v_status NOT IN ('draft','planning') THEN
    RAISE EXCEPTION 'Onda % já iniciada (status=%)', p_wave_id, v_status;
  END IF;
  ...
  -- Abre Corte e Mesa simultaneamente (se existirem)
  UPDATE production_wave_stages
     SET status     = 'in_progress',
         started_at = COALESCE(started_at, now())
   WHERE wave_id = p_wave_id
     AND stage IN ('corte','mesa')
     AND status  = 'pending';
END;
$$;
       ...
       (SELECT to_jsonb(t) FROM (
          SELECT s3.wave_id, s3.wave_code AS code, s3.week_start
           ...
           WHERE s3.stage = stages.stage
             ...
             AND (
               -- Regras de pré-requisito por estágio
               CASE s3.stage
