-- =============================================================================
-- RPC v10: calculate_employee_bank_balance ciente de diarista (meia diária).
-- =============================================================================
-- Mensalista: fluxo semanal normal (v8 + futuro clampado + dia coberto).
-- Diarista: retorna cedo, sem HE/falta/banco. Conta dias trabalhados por carga:
--   >= 6h = 1 diária; 2h..6h = meia diária; < 2h = não conta.
--   daily_pay = (dias_cheios + meios*0.5) * daily_rate.
-- Aplicado em produção via MCP. Idempotente (CREATE OR REPLACE).
-- =============================================================================
...
BEGIN
  ...
  IF v_effective_from > v_effective_to THEN
    RETURN jsonb_build_object(
      ...
      'apuracao', 'semanal_individual_schedule_v10_diarista_aware'
    );
  END IF;
