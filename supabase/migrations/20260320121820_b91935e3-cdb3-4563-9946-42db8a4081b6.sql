...
BEGIN
  ...
  SELECT ts.upper_material, ts.upper_consumption, ts.lining_material, ts.lining_consumption,
         ...
         ts.sole_group_id, ts.lining_accessories
  INTO v_upper_material, v_upper_consumption, v_lining_material, v_lining_consumption,
       ...
       v_sole_group_id, v_lining_accessories
  FROM public.technical_sheets ts
  WHERE ts.id = p_reference_id;
  ...
  -- Sole (Solado) - uses sole_group_id if available, falls back to sole_material name
  IF COALESCE(v_sole_consumption, 0) > 0 AND (v_sole_group_id IS NOT NULL OR (v_sole_material IS NOT NULL AND v_sole_material <> '')) THEN
    required := v_sole_consumption * p_order_quantity;
    target_product_id := NULL;

    -- Try by sole_group_id first (more reliable)
    IF v_sole_group_id IS NOT NULL THEN
      IF p_color IS NOT NULL AND p_color <> '' THEN
        SELECT p.id, p.name, p.quantity INTO target_product_id, target_name, target_qty
        FROM public.products p
        WHERE p.active = true AND p.group_id = v_sole_group_id AND p.color = p_color LIMIT 1;
      END IF;
      IF target_product_id IS NULL THEN
        SELECT p.id, p.name, p.quantity INTO target_product_id, target_name, target_qty
        FROM public.products p
        WHERE p.active = true AND p.group_id = v_sole_group_id LIMIT 1;
      END IF;
    END IF;
    ...
    IF target_product_id IS NOT NULL THEN
      ...
      INSERT INTO public.stock_movements (product_id, movement_type, quantity, previous_stock, new_stock, description, order_id)
      VALUES (target_product_id, 'out', required, target_qty, target_qty - required,
              'Debito Solado (' || target_name || ')' || CASE WHEN p_color <> '' THEN ' Cor: ' || p_color ELSE '' END, p_order_id);
    END IF;
  END IF;

  -- ========== 4) DEBIT direct_components (unit-based) ==========
  ...
  IF v_direct_components IS NOT NULL AND jsonb_typeof(v_direct_components) = 'array' AND jsonb_array_length(v_direct_components) > 0 THEN
    ...
    LOOP
      ...
      IF target_qty < required THEN
        RAISE EXCEPTION 'Estoque insuficiente para componente "%": disponivel %, necessario %',
          target_name, target_qty, required;
      END IF;
