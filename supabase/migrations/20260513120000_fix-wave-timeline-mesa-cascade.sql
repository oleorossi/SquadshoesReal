-- =============================================================================
-- Corrige compute_wave_timeline(): Mesa estava faltando no cálculo de offsets
-- retroativos para corte_forracao_start_date, corte_palmilha_start_date,
-- material_ready_date e purchase_deadline.
--
-- Fluxo correto (backward cascade):
--   Acabamento ← Solagem ← Montagem ← Colagem ← Silk ← Mesa ← Corte Forração ← Corte Palmilha
--
-- Também adiciona mesa_start_date ao RETURN TABLE.
-- =============================================================================
...
COMMENT ON FUNCTION public.compute_wave_timeline(uuid[]) IS
  'Cronograma reverso de uma onda de produção (9 setores completos). '
  'Mesa incluída na cascade: offsets de corte_forracao/palmilha/material_ready/purchase '
  'agora somam v_lead_mesa. mesa_start_date adicionada ao RETURN TABLE. '
  'Fonte de capacidade: ficha técnica > default_lead_times > constante.';
