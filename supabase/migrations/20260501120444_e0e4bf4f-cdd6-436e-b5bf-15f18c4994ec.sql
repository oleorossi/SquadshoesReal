...
-- ---------------------------------------------------------------
-- Update consumption RPCs for 3 production models:
--   Model 1: Corte Cabedal (insole_ready_made=true) → insole in units, skip lining
--   Model 2: Standard (default) → no change
...
DECLARE
  ...
  -- Model detection
  v_is_model1          boolean := false;  -- Corte Cabedal (insole ready-made)
  v_is_model3          boolean := false;  -- Tiras/Mesa
  v_sole_insole_included boolean := false;
BEGIN
  ...
  -- Detect production model
  v_is_model1 := COALESCE(v_sheet.insole_ready_made, false);
  v_is_model3 := COALESCE(v_sheet.mesa_daily_capacity, 0) > 0;
  ...
  -- Check if sole's product group has insole_included
  IF v_sole_product_id IS NOT NULL THEN
    SELECT COALESCE(pg.insole_included, false) INTO v_sole_insole_included
    FROM products p
    JOIN product_groups pg ON pg.id = p.group_id
    WHERE p.id = v_sole_product_id;
  END IF;

  -- Cabedal: skip for Model 3 (Tiras — no cabedal cut)
  IF NOT v_is_model3 AND v_sheet.upper_material IS NOT NULL AND v_sheet.upper_material <> '' THEN
  ...
  -- Palmilha: skip if sole's group has insole_included=true
  -- When insole_ready_made (Model 1), consume in units (1:1), not dm²
  v_palmilha_color := p_color;
  IF NOT v_sole_insole_included
  ...
  FOR v_size, v_pairs IN
    SELECT key::integer, value::numeric
     ...
     WHERE key ~ '^[0-9]+$' AND (value)::numeric > 0
  LOOP
    -- Model 3 skips upper; Model 1 skips lining
    IF NOT v_is_model3 THEN
      v_upper  := NULLIF(COALESCE((v_sheet.upper_consumption_per_size  ->>(v_size::text))::numeric, 0), 0);
    ELSE
      v_upper := 0;
    END IF;

    IF NOT v_is_model1 THEN
      v_lining := NULLIF(COALESCE((v_sheet.lining_consumption_per_size ->>(v_size::text))::numeric, 0), 0);
    ELSE
      v_lining := 0;
    END IF;

    -- Model 1 with insole_ready_made: insole consumed 1:1 in units, not dm²
    IF v_is_model1 AND NOT v_sole_insole_included THEN
      v_insole := 1; -- 1 unit per pair
    ELSIF NOT v_sole_insole_included THEN
      v_insole := NULLIF(COALESCE((v_sheet.insole_consumption_per_size ->>(v_size::text))::numeric, 0), 0);
    ELSE
      v_insole := 0;
    END IF;

    -- Fallback to sole_technical_specs for missing per-size values (not for Model 1 insole or Model 3 upper)
    IF (v_upper IS NULL OR v_lining IS NULL OR v_insole IS NULL)
      ...
      IF FOUND THEN
        IF v_upper  IS NULL AND NOT v_is_model3 AND COALESCE(v_spec.upper_consumption_dm2,  0) > 0 THEN v_upper  := v_spec.upper_consumption_dm2;  END IF;
        IF v_lining IS NULL AND NOT v_is_model1 AND COALESCE(v_spec.lining_consumption_dm2, 0) > 0 THEN v_lining := v_spec.lining_consumption_dm2; END IF;
        IF v_insole IS NULL AND NOT v_sole_insole_included AND COALESCE(v_spec.insole_consumption_dm2, 0) > 0 THEN v_insole := v_spec.insole_consumption_dm2; END IF;
      END IF;
    END IF;
  ...
  -- Forro (skipped for Model 1 and when insole_has_lining=false)
  ...
  -- Palmilha: Model 1 consumes in units (debit_mode=hard), others in dm² (soft)
  IF v_insole_pid IS NOT NULL AND COALESCE((v_acc_insole->>'required')::numeric, 0) > 0 THEN
    ...
    v_result := v_result || jsonb_build_object(
      ...
      'required', v_required, 'available', v_resolved.available_qty,
      'stock_ok', v_resolved.available_qty >= v_required,
      'debit_mode', CASE WHEN v_is_model1 THEN 'hard' ELSE 'soft' END,
      'source', CASE WHEN v_is_model1 THEN 'insole_ready_made' ELSE 'sheet_per_size' END,
  ...
  -- Forros alternativos (lining_accessories) — skipped for Model 1 and insole_has_lining=false
  IF NOT v_is_model1
     AND COALESCE(v_sheet.insole_has_lining, true) = true
...
DECLARE
  ...
  v_palmilha_color     text;
  -- Model detection
  v_is_model1          boolean := false;
  v_is_model3          boolean := false;
  v_sole_insole_included boolean := false;
BEGIN
  SELECT * INTO v_sheet FROM technical_sheets WHERE id = p_reference_id;
  IF NOT FOUND THEN
  ...
  -- Check insole_included on sole's product group
  IF v_sole_product_id IS NOT NULL THEN
    SELECT COALESCE(pg.insole_included, false) INTO v_sole_insole_included
    FROM products p
    JOIN product_groups pg ON pg.id = p.group_id
  ...
  -- Resolve consumptions based on model
  IF NOT v_is_model3 THEN
  ...
  IF v_is_model1 AND NOT v_sole_insole_included THEN
    v_insole_consumption := 1; -- units, not dm²
  ELSIF NOT v_sole_insole_included THEN
  ...
  -- Fallback to sole_technical_specs
  IF (v_upper_consumption IS NULL OR v_lining_consumption IS NULL OR v_insole_consumption IS NULL)
    ...
    IF FOUND THEN
      IF v_upper_consumption  IS NULL AND NOT v_is_model3 AND COALESCE(v_spec.upper_consumption_dm2,  0) > 0 THEN v_upper_consumption  := v_spec.upper_consumption_dm2;  END IF;
      IF v_lining_consumption IS NULL AND NOT v_is_model1 AND COALESCE(v_spec.lining_consumption_dm2, 0) > 0 THEN v_lining_consumption := v_spec.lining_consumption_dm2; END IF;
      IF v_insole_consumption IS NULL AND NOT v_sole_insole_included AND COALESCE(v_spec.insole_consumption_dm2, 0) > 0 THEN v_insole_consumption := v_spec.insole_consumption_dm2; END IF;
    END IF;
  END IF;
  ...
  END IF;
  IF v_is_model1 AND NOT v_sole_insole_included THEN
    v_insole_consumption := COALESCE(v_insole_consumption, 1);
  ELSIF NOT v_sole_insole_included THEN
  ...
  IF v_sole_product_id IS NOT NULL THEN
    ...
    -- Fachete (only if not Model 3 — needs lining material)
    IF NOT v_is_model3 THEN
  ...
  -- Cabedal (skipped for Model 3)
  IF NOT v_is_model3
     AND v_sheet.upper_material IS NOT NULL AND v_sheet.upper_material <> ''
  ...
  -- Forração (skipped for Model 1 and when insole_has_lining=false)
  IF NOT v_is_model1
  ...
  -- Palmilha: Model 1 uses hard debit (units), others use soft (dm²)
  IF NOT v_sole_insole_included
     AND v_sheet.insole_material IS NOT NULL AND v_sheet.insole_material <> ''
    ...
    IF v_resolved.product_id IS NOT NULL THEN
      IF v_is_model1 THEN
        -- Model 1: insole consumed in units (1:1), no dm² conversion
        v_result := v_result || jsonb_build_object(
          'component', 'Palmilha', 'product_id', v_resolved.product_id, 'product_name', v_resolved.product_name,
          'color', v_palmilha_color, 'consumption_per_unit', 1,
          'required', v_required, 'available', v_resolved.available_qty,
          'stock_ok', v_resolved.available_qty >= v_required, 'debit_mode', 'hard',
          'source', 'insole_ready_made', 'matched_by', v_resolved.matched_by);
      ELSE
        SELECT * INTO v_conv FROM get_material_conversion_info(v_resolved.product_id);
        v_required := (v_required / NULLIF(v_conv.dm2_per_unit, 0)) * (1 + v_conv.waste_pct / 100);
        v_result := v_result || jsonb_build_object(
