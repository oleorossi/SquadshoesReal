-- Fix #1 CRÍTICO (auditoria unidades 2026-06-07): o bloco lining_accessory
-- ("Forração alternativa") em calculate_order_consumption E
-- calculate_order_consumption_by_grade emitia dm² CRU (sem get_material_conversion_info,
-- sem campo 'unit') → custeio inflado 137× p/ napa (ex.: NAPA SOFT R$7.617 vs R$55).
-- Todos os outros blocos de área já convertiam; só lining_accessory faltava nas duas.
-- Agora ambas convertem dm²→unidade física + emitem 'unit', espelhando o bloco Forração.
-- Aplicado via MCP e validado (PV 3f5d2ca1: R$105 → R$4,63/par).
