-- Fix MÉDIO (auditoria 2026-06-06): get_purchase_projection contava OCs status='suggested'
-- (sugestões automáticas do ROP, NÃO colocadas) como compras reais → poluía ABC, valor
-- comprado, preço médio. Exclui 'suggested' do CTE purchases (mantém pending/approved/etc.).
...
BEGIN
  ...
  purchases AS (
    ...
    WHERE po.created_at >= now() - (p_days || ' days')::interval
      AND COALESCE(po.status,'pending') NOT IN ('cancelled','cancelado','suggested')
  ...
  enriched AS (
    SELECT
      ...
      COALESCE(p.color,'') AS pcolor, COALESCE(p.unit,'un') AS punit,
      ...
      p.supplier_id AS p_supid, sup.name AS p_supname,
  ...
  SELECT
    e.pid, e.pname, e.pcat, e.pcolor, e.punit, e.p_is_artisanal, e.p_stock, e.p_reserved, e.p_avail,
    e.p_pqty, e.p_pval, e.p_pcount, e.p_plast, e.p_pavg, e.p_cqty,
    ROUND(e.p_avg_daily, 3) AS avg_daily_consumption,
    CASE WHEN e.p_avg_daily > 0 THEN ROUND(e.p_avail / e.p_avg_daily, 1) ELSE NULL END AS days_of_cover,
    e.p_supid, e.p_supname, e.p_lead, e.p_abc, e.p_abc_share,
    CEIL(e.p_avg_daily * e.p_lead * v_safety_factor)::numeric AS suggested_min_stock,
    GREATEST(0, CEIL(e.p_avg_daily * (e.p_lead * v_safety_factor + v_target_cover_days)) - e.p_avail)::numeric AS suggested_reorder_qty,
    CASE
