-- =============================================================================
-- 20260512200000 — Fix: material_cost subestimado quando qty > grade_sum
-- =============================================================================
-- Bug: a função calculate_order_consumption_by_grade retorna consumo baseado
-- nas quantidades literais da grade (ex.: grade {34:1,35:2,...} soma 12 pares).
-- A função calculate_order_cost_item somava o subtotal direto, sem multiplicar
-- pelo ratio (qty/grade_sum). Em OP de 360 pares com grade que soma 12 (i.e.,
-- 30 dúzias da mesma grade), o material_cost era armazenado pra 12 pares
-- enquanto overhead+packaging eram pra 360 pares — total inconsistente, e
-- displays de consumo (UI) podiam multiplicar por qty gerando inflação 30×.
--
-- Fix: calcula grade_sum no início, deriva qty_multiplier = qty / grade_sum,
-- e aplica ao required + subtotal de cada linha. Idempotente quando qty
-- == grade_sum (multiplier = 1). Inclui qty_multiplier e grade_sum no
-- breakdown pra auditoria.
--
-- Caveat: a fix afeta APENAS calculate_order_cost_item (custo). As funções
-- de débito de estoque (debit_strap_stock, debit_sole_stock_by_grade, etc.)
-- continuam consumindo a grade como vem do PV — se o débito hoje está OK
-- na realidade física, é porque o caller já multiplica antes ou o grade
-- representa o total. Esta migration não muda esse comportamento.
-- =============================================================================
