-- =============================================================================
-- EQUIPE & CAPACIDADE — motor v5: camada MEDIDA na cadeia de minutos
-- (specs/equipe-e-capacidade-fonte-unica.md — R12, R21, R28, R29, R30)
--
-- ⚠ Esta versão foi aplicada via MCP durante o build e ficou SEM arquivo de
-- migration até esta correção: o repo tinha o v4 (sem camada medida) enquanto o
-- banco já rodava o v5. Recriar o ambiente a partir do repo derrubaria
-- `somente_padrao` e a camada 'medido', que o frontend consome.
--
-- Cadeia: BOM do modelo (manual/crono/capacidade) > última referência >
--         MEDIDO (soma das pessoas do setor) > padrão da categoria.
-- A ficha MANDA quando tem valor (R21); a medição é a ESCALA do setor (R30);
-- capacidade observada não sofre eficiência (R28); custo-hora vem do RH (R29).
-- Medição pré-calculada 1× por chamada (evita N×M varreduras em employees).
-- =============================================================================
...
BEGIN
  ...
  FOR v_ss IN SELECT sector FROM sector_settings WHERE enabled LOOP
    v_measured_map := v_measured_map || jsonb_build_object(
      ...
      public.sector_measured_capacity(v_ss.sector, 3650));
  END LOOP;
  ...
  FOREACH v_id IN ARRAY p_sheet_ids LOOP
    ...
    LOOP
      ...
      ELSE
        IF v_source IN ('default', 'medido') THEN
          v_mo_sector := round(v_minutes * v_rate / 60.0, 4);
        END IF;
        IF v_rate = 0 AND coalesce(v_mo_sector, 0) = 0 THEN
          v_warnings := v_warnings ||
            format('%s: sem custo-hora (nem RH nem cadastro) — MO do setor ficou R$ 0', v_rec.label);
        END IF;
