...
-- Add group_id to component_sheets
...
-- Add group_id to sheet_materials
ALTER TABLE public.sheet_materials
  ADD COLUMN IF NOT EXISTS group_id uuid REFERENCES public.product_groups(id) ON DELETE SET NULL;

-- Migrate existing data: fill group_id from products.group_id
UPDATE public.component_sheets cs
SET group_id = p.group_id
FROM public.products p
WHERE cs.product_id = p.id AND p.group_id IS NOT NULL AND cs.group_id IS NULL;
...
DECLARE
  ...
  v_spec_label text;
  v_resolve_group_id uuid;
BEGIN
  -- ========== 1) DEBIT sheet_materials (BOM) ==========
  FOR mat IN
    SELECT sm.product_id, sm.quantity_per_unit, sm.group_id AS sm_group_id,
           p.quantity AS current_stock, p.name, p.group_id AS p_group_id, p.color AS product_color
  ...
  LOOP
    ...
    v_resolve_group_id := COALESCE(mat.sm_group_id, mat.p_group_id);
    ...
    IF p_color IS NOT NULL AND p_color <> '' AND mat.product_color <> p_color THEN
      ...
      WHERE p.active = true AND p.color = p_color
        AND ((v_resolve_group_id IS NOT NULL AND p.group_id = v_resolve_group_id)
             OR (v_resolve_group_id IS NULL AND p.name = mat.name))
      LIMIT 1;
    ...
    LOOP
      ...
      ELSE
        ...
        IF v_group_name IS NOT NULL AND v_group_name <> '' THEN
          ...
          WHERE pg.name = v_group_name AND p.active = true
            AND (p_color = '' OR p.color = p_color OR p.color IS NULL OR p.color = '')
          ORDER BY CASE WHEN p.color = p_color THEN 0 ELSE 1 END LIMIT 1;
          IF target_product_id IS NULL THEN
            RAISE EXCEPTION 'Componente "%" nao encontrado no estoque', v_group_name;
          END IF;
        ELSE CONTINUE;
        END IF;
      END IF;
      ...
      VALUES (target_product_id, 'out', required, target_qty, target_qty - required,
              'Debito Extra: ' || target_name || CASE WHEN p_color <> '' THEN ' (Cor: ' || p_color || ')' ELSE '' END, p_order_id);
    END LOOP;
  END IF;

  -- ========== 3) DEBIT specification fields ==========
  SELECT ts.upper_material, ts.upper_consumption, ts.lining_material, ts.lining_consumption,
         ts.insole_material, ts.insole_consumption, ts.sole_material, ts.sole_consumption, ts.lining_accessories
  INTO v_upper_material, v_upper_consumption, v_lining_material, v_lining_consumption,
       v_insole_material, v_insole_consumption, v_sole_material, v_sole_consumption, v_lining_accessories
  FROM public.technical_sheets ts WHERE ts.id = p_reference_id;

  FOR v_spec_label IN SELECT unnest(ARRAY['Cabedal', 'Forro', 'Palmilha', 'Solado'])
  LOOP
    IF v_spec_label = 'Cabedal' AND v_upper_material IS NOT NULL AND v_upper_material <> '' AND v_upper_consumption > 0 THEN
      v_group_name := v_upper_material; v_consumption := v_upper_consumption;
    ELSIF v_spec_label = 'Forro' AND v_lining_material IS NOT NULL AND v_lining_material <> '' AND v_lining_consumption > 0 THEN
      v_group_name := v_lining_material; v_consumption := v_lining_consumption;
    ELSIF v_spec_label = 'Palmilha' AND v_insole_material IS NOT NULL AND v_insole_material <> '' AND v_insole_consumption > 0 THEN
      v_group_name := v_insole_material; v_consumption := v_insole_consumption;
    ELSIF v_spec_label = 'Solado' AND v_sole_material IS NOT NULL AND v_sole_material <> '' AND v_sole_consumption > 0 THEN
      v_group_name := v_sole_material; v_consumption := v_sole_consumption;
    ELSE CONTINUE;
    END IF;
    ...
    ORDER BY CASE WHEN p.color = p_color THEN 0 ELSE 1 END LIMIT 1;
    IF target_product_id IS NULL THEN
      RAISE EXCEPTION 'Material "%" (%) nao encontrado no estoque', v_group_name, v_spec_label;
    END IF;
    IF target_qty < required THEN
      RAISE EXCEPTION 'Estoque insuficiente para "%" (%): disponivel %, necessario %', target_name, v_spec_label, target_qty, required;
    END IF;
    UPDATE public.products SET quantity = quantity - required, updated_at = now() WHERE id = target_product_id;
    INSERT INTO public.stock_movements (product_id, movement_type, quantity, previous_stock, new_stock, description, order_id)
    VALUES (target_product_id, 'out', required, target_qty, target_qty - required,
            'Debito ' || v_spec_label || ': ' || v_group_name || CASE WHEN p_color <> '' THEN ' (Cor: ' || p_color || ')' ELSE '' END, p_order_id);
  END LOOP;

  -- ========== 3b) DEBIT lining_accessories ==========
  IF v_lining_accessories IS NOT NULL AND jsonb_typeof(v_lining_accessories) = 'array' AND jsonb_array_length(v_lining_accessories) > 0 THEN
    ...
    LOOP
      ...
      ORDER BY CASE WHEN p.color = p_color THEN 0 ELSE 1 END LIMIT 1;
      IF target_product_id IS NULL THEN
        RAISE EXCEPTION 'Material de forro "%" nao encontrado no estoque', v_group_name;
      END IF;
      IF target_qty < required THEN
      ...
      INSERT INTO public.stock_movements (product_id, movement_type, quantity, previous_stock, new_stock, description, order_id)
      VALUES (target_product_id, 'out', required, target_qty, target_qty - required,
              'Debito Forro: ' || v_group_name || CASE WHEN p_color <> '' THEN ' (Cor: ' || p_color || ')' ELSE '' END, p_order_id);
    END LOOP;
  END IF;

  -- ========== 4) DEBIT strap materials ==========
  PERFORM public.debit_strap_materials(p_reference_id, p_order_quantity, p_color, p_order_id);
END;
$$;
