-- =============================================================================
-- PR 3 — Paralelismo: Corte Palmilha ‖ Corte Forração ‖ Aviamento
-- =============================================================================
-- Decisão fechada com o usuário: APENAS estes 3 setores rodam em paralelo.
-- Costura, Silk, Colagem, Montagem, Solagem, Acabamento são SEQUENCIAIS.
--
-- Modelo de cascata novo:
--   Phase 1 (paralelo): MAX(Corte Palmilha, Corte Forração, Aviamento)
--   Phase 2+: Costura → Silk → Colagem → Montagem → Solagem → Acabamento
--
-- Lead time total da onda:
--   prep_lead = GREATEST(palmilha, forracao, mesa)
--   total = prep_lead + costura + silk + colagem + montagem + solagem + acab
--
-- Cada setor de prep começa em SUA própria data:
--   corte_palmilha_start = costura_start - v_lead_palmilha
--   corte_forracao_start = costura_start - v_lead_forracao
--   mesa_start           = costura_start - v_lead_mesa
--
-- Isso significa que os 3 podem começar em datas DIFERENTES (quem tem mais
-- volume começa antes), mas TODOS terminam na MESMA data — quando o Costura
-- arranca.
--
-- material_ready_date e purchase_deadline usam a data MAIS ANTIGA (de quem
-- começa primeiro), pra garantir que o material esteja disponível antes do
-- início mais cedo entre os 3 prep sectors.
-- =============================================================================
...
DECLARE
  ...
  v_lead_supplier  int;
  v_deadline       date;
  v_lead_prep_max  int;        -- MAX dos 3 paralelos
  v_post_prep      int;        -- soma sequencial pós prep
  v_costura_start  date;       -- ponto de convergência dos 3 prep
  v_earliest_prep  date;       -- data mais antiga (pra material_ready)
BEGIN
  ...
  SELECT
    ...
    -- Mesa/Aviamento (paralelo com prep)
    COALESCE(MAX(
      CASE WHEN EXISTS (
        SELECT 1 FROM jsonb_array_elements_text(COALESCE(ts.production_sectors,'[]'::jsonb)) x
  ...
  -- ── Cascada com paralelismo ────────────────────────────────────────────
  -- post_prep = soma sequencial DEPOIS do prep
  v_post_prep := v_lead_acab + v_lead_solagem + v_lead_montagem
                + v_lead_colagem + v_lead_silk + v_lead_costura;

  -- prep_max = lead time do mais lento entre os 3 paralelos
  v_lead_prep_max := GREATEST(v_lead_palmilha, v_lead_forracao, v_lead_mesa);

  -- Costura começa quando os 3 prep terminaram (ponto de convergência)
  v_costura_start := add_business_days(v_deadline, -v_post_prep)::date;

  -- Cada prep começa em SUA própria data, terminando todos em v_costura_start
  -- A data mais antiga (de quem começa primeiro) define material_ready
  v_earliest_prep := add_business_days(v_costura_start, -v_lead_prep_max)::date;

  RETURN QUERY SELECT
    v_deadline AS earliest_deadline,

    -- Cada prep sector tem SUA OWN start date (back-calculated do costura_start)
    ...
    -- material_ready = início mais antigo dos prep − buffer
...
COMMENT ON FUNCTION public.compute_wave_timeline(uuid[]) IS
  'Cascata com paralelismo: Corte Palmilha ‖ Corte Forração ‖ Aviamento (mesa) '
  'rodam em paralelo, terminam todos no costura_start. Costura → Silk → Colagem '
  '→ Montagem → Solagem → Acabamento são sequenciais. material_ready usa o início '
  'MAIS ANTIGO entre os 3 prep para garantir disponibilidade de matéria-prima '
