-- Fix requisito #3 (auditoria JIT 2026-06-06): check_stock_availability retornava
-- available = p.quantity (BRUTO), ignorando reserved_stock. Na criação do PV, um material
-- fisicamente presente mas todo reservado pra outro pedido aparecia como "disponível".
-- O usuário pediu explicitamente avaliar "se tenho disponível OU se está reservada".
-- Agora available = GREATEST(0, quantity - reserved_stock) (líquido). Seguro: a função é
-- só de leitura/checagem (nenhum débito depende dela; débito usa hybrid_debit_stock_for_order)
-- e solados não passam por aqui (vão por stock_grade/debit_sole_stock_by_grade).
