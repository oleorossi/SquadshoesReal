-- ============================================================================
-- M16 (auditoria 2026-07-28) — motor de agenda diária ignora a capacidade de
-- ficha das duas Costuras.
--
-- A 20261001120000 apontou sector_settings.ficha_capacity_column para
-- 'costura_palmilha_capacity_per_day'/'costura_cabedal_capacity_per_day', mas
-- a recompute_production_schedule viva (20260920103000) resolvia o override de
-- ficha num CASE que só conhecia as colunas antigas (ELSE NULL pra coluna
-- desconhecida): Costura Palmilha/Cabedal eram agendadas pelo default global
-- do setor (daily_capacity_pairs), regressão vs. pré-split.
--
-- Fix (diff mínimo sobre a definição viva): a CTE ficha busca as duas colunas
-- novas e o CASE ganha os dois WHEN, com fallback na legada
-- costura_capacity_per_day (espelho de src/lib/leadTime.ts). Todo o resto da
-- função é idêntico. Idempotente (CREATE OR REPLACE).
-- ============================================================================
...
BEGIN
  ...
  WITH ficha AS (
    SELECT DISTINCT ON (o.id)
           ...
           ts.costura_palmilha_capacity_per_day, ts.costura_cabedal_capacity_per_day,
  ...
  enriched AS (
    SELECT b.order_id, b.sector, b.remaining, b.processed,
           ...
           CASE ss.ficha_capacity_column
             ...
             -- M16: colunas do split da Costura (20261001120000), com fallback
             -- na legada enquanto as fichas não têm os dois campos preenchidos.
             WHEN 'costura_palmilha_capacity_per_day' THEN COALESCE(
               NULLIF(COALESCE(f.costura_palmilha_capacity_per_day::numeric, 0), 0),
               NULLIF(COALESCE(f.costura_capacity_per_day::numeric, 0), 0))
             WHEN 'costura_cabedal_capacity_per_day'  THEN COALESCE(
               NULLIF(COALESCE(f.costura_cabedal_capacity_per_day::numeric, 0), 0),
               NULLIF(COALESCE(f.costura_capacity_per_day::numeric, 0), 0))
