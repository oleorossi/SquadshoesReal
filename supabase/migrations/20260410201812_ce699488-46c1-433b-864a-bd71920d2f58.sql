...
DECLARE
  ...
  v_sole_material text; v_sole_consumption numeric;
...
BEGIN
  ...
  FOR mat IN
    SELECT sm.product_id, sm.quantity_per_unit, sm.consumption_per_size,
           p.quantity AS current_stock, p.name, p.group_id, p.color AS product_color, p.category, p.stock_grade
    FROM public.sheet_materials sm
    JOIN public.products p ON p.id = sm.product_id
    WHERE sm.sheet_id = p_reference_id
  LOOP
    -- Compute required and scaled grade for stock_grade update
    v_cps := mat.consumption_per_size;
    v_scaled_grade := '{}'::jsonb;
    
    IF p_order_grade IS NOT NULL AND jsonb_typeof(p_order_grade) = 'object' AND p_order_grade != '{}'::jsonb THEN
      v_computed := 0;
      FOR v_size IN SELECT jsonb_object_keys(p_order_grade)
      LOOP
        v_grade_qty := COALESCE((p_order_grade ->> v_size)::numeric, 0);
        v_size_consumption := 0;
        
        IF v_cps IS NOT NULL AND jsonb_typeof(v_cps) = 'object' AND (v_cps ->> v_size) IS NOT NULL THEN
        ...
        v_scaled_grade := v_scaled_grade || jsonb_build_object(v_size, v_size_consumption * v_grade_qty);
      END LOOP;
      required := v_computed;
    ELSE
    ...
    IF p_color IS NOT NULL AND p_color <> '' AND mat.product_color <> p_color THEN
      SELECT p.id, p.name, p.quantity, p.category, p.stock_grade 
      INTO target_product_id, target_name, target_qty, v_category, mat.stock_grade
    ...
    IF v_is_immediate THEN
      ...
      SET 
        ...
        stock_grade = (
          SELECT jsonb_object_agg(key, COALESCE((val)::numeric, 0) - COALESCE((v_scaled_grade ->> key)::numeric, 0))
          FROM jsonb_each(COALESCE(stock_grade, '{}'::jsonb)) AS t(key, val)
        ),
        updated_at = now() 
  ...
  -- ========== 2) PROCESS specifications (Cabedal, Forro, Palmilha, Solado) ==========
  ...
  -- (Omitting Lining Accessories logic for brevity if needed, but I should keep it for correctness)
  -- Actually, let's keep the full logic to avoid breaking things.
  ...
  -- Solado (Sole) - HARD reservation (Added section)
  IF v_sole_material IS NOT NULL AND v_sole_material <> '' AND COALESCE(v_sole_consumption, 0) > 0 THEN
    -- For solado, we usually assume 1 per size if consumption is 1. We scale based on p_order_grade.
    v_scaled_grade := '{}'::jsonb;
    IF p_order_grade IS NOT NULL AND jsonb_typeof(p_order_grade) = 'object' AND p_order_grade != '{}'::jsonb THEN
      v_computed := 0;
      FOR v_size IN SELECT jsonb_object_keys(p_order_grade)
      LOOP
        v_grade_qty := COALESCE((p_order_grade ->> v_size)::numeric, 0);
        v_computed := v_computed + (v_sole_consumption * v_grade_qty);
        v_scaled_grade := v_scaled_grade || jsonb_build_object(v_size, v_sole_consumption * v_grade_qty);
      END LOOP;
      required := v_computed;
    ELSE
    ...
    target_product_id := NULL;
    IF p_color <> '' THEN
      SELECT p.id, p.name, p.quantity, p.stock_grade INTO target_product_id, target_name, target_qty, mat.stock_grade
      FROM products p JOIN product_groups pg ON pg.id = p.group_id
    ...
    IF target_product_id IS NULL AND p_color <> '' THEN
      SELECT p.id, p.name, p.quantity, p.stock_grade INTO target_product_id, target_name, target_qty, mat.stock_grade
      FROM products p JOIN product_groups pg ON pg.id = p.group_id
      WHERE p.active = true AND pg.name = v_sole_material AND LOWER(p.name) LIKE '%' || LOWER(p_color) || '%' LIMIT 1;
    END IF;
    IF target_product_id IS NULL THEN
      SELECT p.id, p.name, p.quantity, p.stock_grade INTO target_product_id, target_name, target_qty, mat.stock_grade
      FROM products p JOIN product_groups pg ON pg.id = p.group_id
    ...
    IF target_product_id IS NOT NULL THEN
      IF target_qty < required THEN
        RAISE EXCEPTION 'Estoque insuficiente para Solado "%": disponivel %, necessario %', target_name, target_qty, required;
      END IF;
      
      -- Update stock and grade
      UPDATE public.products 
      SET 
        quantity = quantity - required, 
        stock_grade = (
      ...
      VALUES (target_product_id, 'out', required, target_qty, target_qty - required, 'Debito Solado (' || target_name || ')', p_order_id);
  ...
  IF v_direct_components IS NOT NULL AND jsonb_typeof(v_direct_components) = 'array' AND jsonb_array_length(v_direct_components) > 0 THEN
    ...
    LOOP
      ...
      IF p_order_grade IS NOT NULL AND jsonb_typeof(p_order_grade) = 'object' AND p_order_grade != '{}'::jsonb THEN
        ...
        LOOP
          v_grade_qty := COALESCE((p_order_grade ->> v_size)::numeric, 0);
          v_computed := v_computed + (v_consumption * v_grade_qty);
          v_scaled_grade := v_scaled_grade || jsonb_build_object(v_size, v_consumption * v_grade_qty);
        END LOOP;
        required := v_computed;
      ELSE
