-- =============================================================================
-- Fix calculate_order_consumption_by_grade: missing dm² conversion + waste
-- =============================================================================
--
-- Migration 20260501120000 simplified the graded function and ACCIDENTALLY
-- removed the unit conversion + waste percentage logic for Cabedal/Forro/
-- Palmilha. The accumulator stores raw dm² × pairs, but the result emission
-- forgot to:
--
--   1. Convert dm² → target unit (m for fabrics, ml for paint, …) by
--      dividing by `dm2_per_unit` from get_material_conversion_info().
--   2. Apply the waste percentage `(1 + waste_pct/100)` multiplier.
--   3. Include the `unit` field in the JSONB output, breaking UI rendering.
--
-- The single-size function (calculate_order_consumption) in the same migration
-- correctly applies both, and the older migration 20260426120000 had it
-- working in the graded function before the regression.
--
-- IMPACT: graded orders systematically UNDER-estimated material consumption
-- by 5-15% (waste) plus the unit-conversion factor. Reports could show
-- stock_ok=true when stock was actually insufficient, and the MRP could
-- under-purchase materials.
--
-- This migration restores the conversion+waste pipeline in the graded path.
-- =============================================================================
...
DECLARE
  ...
  v_insole             numeric;
  v_resolved           RECORD;
  v_conv               RECORD;            -- restored: conversion info
  v_row                RECORD;
  v_item               jsonb;
...
BEGIN
  ...
  -- Standard sole items (already in correct unit from sole_standard_items_consumption.unit)
  ...
  -- BOM legado (sheet_materials) — quantity_per_unit já está em unidade final
  FOR v_row IN
  ...
  -- Forros alternativos (lining_accessories) — consumo dado já em unidade final
