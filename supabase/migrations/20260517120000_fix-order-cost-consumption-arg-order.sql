-- =============================================================================
-- Fix calculate_order_cost: wrong argument order for calculate_order_consumption_by_grade
-- =============================================================================
-- Migration 20260504120000 introduced calculate_order_cost calling:
--   calculate_order_consumption_by_grade(v_ref, COALESCE(v_color,''), v_grade)
--   → resolves to overload (uuid, text, jsonb) — the OLD pre-Group23 version
--
-- Migration 20260510140000 rewrote the function as (uuid, jsonb, text), adding
-- dm² conversion and waste_pct.  calculate_order_cost never called the fixed
-- version — it always hit the legacy overload, so margin was still under-reported
-- by 5-15%.
--
-- Fix:  swap args → (v_ref, v_grade, COALESCE(v_color,''))
--
-- Also adds packaging_cost column to order_costs so the packaging component has
-- its own audit column (previously only inside total_cost with no separate row).
-- =============================================================================

-- 1. Add packaging_cost column (idempotent)
...
DECLARE
  ...
  v_out jsonb;
BEGIN
  -- Load sale order item (filter by item id if provided)
  ...
  -- Consumption (snapshot > live calculation)
  ...
  IF v_cons IS NULL THEN
    IF v_grade IS NOT NULL AND v_grade <> '{}'::jsonb THEN
      -- FIXED: correct arg order (uuid, jsonb, text) matching the Group-23 overload
...
COMMENT ON FUNCTION public.calculate_order_cost(uuid, uuid, boolean) IS
  'Calculates order cost. Uses calculate_order_consumption_by_grade(uuid,jsonb,text) — Group-23 overload with dm² conversion + waste%.';
