...
AS $$
  SELECT lower(translate(coalesce(p_category, ''),
    'áàâãäéèêëíìîïóòôõöúùûüçÁÀÂÃÄÉÈÊËÍÌÎÏÓÒÔÕÖÚÙÛÜÇ',
    'aaaaaeeeeiiiiooooouuuucAAAAAEEEEIIIIOOOOOUUUUC'
  )) IN (
    'acessorio', 'acessório',
    'embalagem',
    'cola', 'adesivo', 'quimico', 'químico', 'cola / quimico', 'cola / químico',
    'ferramentas', 'ferramenta',
    'solado',
    'palmilha',
    'forracao da palmilha', 'forração da palmilha',
    'outros',
...
BEGIN
  ...
  LOOP
    ...
    v_category := COALESCE(mat.category, '');
    ...
    v_is_immediate := public._is_immediate_debit_category(v_category);
    ...
    IF v_is_immediate THEN
      ...
      VALUES (target_product_id, 'out', required, target_qty, target_qty - required,
              'Debito OP (componente) ' || target_name || CASE WHEN p_color <> '' THEN ' Cor: ' || p_color ELSE '' END, p_order_id);
    END IF;
  ...
  -- ========== 2) Cabedal / Forro / Palmilha (soft reservation, color-dependent) ==========
  ...
  -- Lining + accessories handling (preserved)
  ...
  IF NOT v_lining_debited AND v_lining_accessories IS NOT NULL AND jsonb_typeof(v_lining_accessories) = 'array' THEN
  ...
  -- ========== 3) direct_components (HARD debit) ==========
  SELECT ts.direct_components INTO v_direct_components FROM technical_sheets ts WHERE ts.id = p_reference_id;
  IF v_direct_components IS NOT NULL AND jsonb_typeof(v_direct_components) = 'array' THEN
  ...
  -- ========== 4) components_accessories (HARD debit) ==========
...
BEGIN
  ...
  LOOP
    ...
    IF v_is_immediate THEN
      ...
      UPDATE products SET quantity = quantity - required, updated_at = now() WHERE id = target_product_id;
      INSERT INTO stock_movements (product_id, movement_type, quantity, previous_stock, new_stock, description, order_id)
      VALUES (target_product_id, 'out', required, target_qty, target_qty - required, 'Debito OP (componente) ' || target_name, p_order_id);
      INSERT INTO material_reservations (order_id, product_id, quantity_reserved, quantity_consumed, status, reservation_type)
      VALUES (p_order_id, target_product_id, required, required, 'consumed', 'hard');
      v_result := v_result || jsonb_build_object('product_id', target_product_id, 'product_name', target_name, 'required', required, 'type', 'debited');
    ELSE
