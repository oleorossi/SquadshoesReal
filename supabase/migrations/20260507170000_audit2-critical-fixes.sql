...
-- =============================================================================
--
-- Audit Agent1 #10 (CRITICAL): resync_op_atomic recreates order_stages from
-- technical_sheets.production_sectors with a fallback ARRAY of legacy sector
-- names ('Corte','Forração','Aviamento',...) — these were renamed in
-- 20260506120000_sector-rename-wave-stages.sql to ('Corte Palmilha',
-- 'Corte Forração','Mesa',...). Any OP whose sheet has empty production_sectors
-- now gets stages with mixed-vocabulary enum values, breaking stage ordering.
--
-- Audit Agent2 #2 (CRITICAL): autoCreateMaterialPO/autoCreateSolePO call
-- upsert_po_item_atomic followed by a SEPARATE increment_purchase_order_total
-- RPC. Two threads accumulating into the same PO can interleave, leaving the
-- PO total out of sync with the sum of items. Fold the total update into
-- upsert_po_item_atomic so item + total move atomically.
-- =============================================================================
...
-- The previous version returned qty_delta_applied to the caller, who then
-- issued increment_purchase_order_total separately. By folding the total
-- update inside the same transaction (and same SELECT FOR UPDATE-protected
-- block), concurrent callers see a coherent (item, total) pair.
...
BEGIN
  -- Lock PO header so total_value update sees a serialized view alongside
  -- the items lock below.
  ...
  -- Atomic total update — same transaction as the item upsert.
