-- =============================================================================
-- PV edit: 1ª remoção hard-delete quando seguro (sem soft-preserve por demanda)
-- =============================================================================
-- Sintoma (PV-00168 /sales/edit/…, S-039 OFF WHITE / PRETO, M100 NEW WHISKY):
--   Excluir o item no editor → linha vira "RETIRADO DA PRODUÇÃO" com
--   "Apagar de vez". Dono: "Já deveria apagar direto quando eu excluir".
--
-- Causa:
--   finalize_removed (desde 22000) soft-preservava QUALQUER item com linha em
--   sale_order_strap_demands / purchase_demand_contributions. Isso era necessário
--   quando a FK era ON DELETE RESTRICT. Em 22100 a FK virou ON DELETE SET NULL
--   e 23300/23500 já hard-deletam na 2ª remoção — a 1ª ficou como atrito morto.
--
-- Correção:
--   Soft-preserve SOMENTE com compra de tira avançada (approved/sent/partial/
--   received) — aí a linha comercial precisa ficar. Nos demais casos (demanda
--   histórica, contribuição reversível/terminal, já soft-excluded) → hard-delete
--   na 1ª remoção. Antes do DELETE, cancela compra reversível + órfão de OC
--   (mesmo bloco da 22200/22900/23000), senão o BEFORE DELETE barra.
--
-- Marcador: pv_edit_hard_delete_first_removal_20270101026300
-- =============================================================================
...
BEGIN
  ...
  -- pv_edit_hard_delete_first_removal_20270101026300
  ...
  LOOP
    ...
    -- Único soft-preserve restante: compra avançada amarra histórico comercial.
    IF EXISTS (
      SELECT 1
    ...
    -- Demanda histórica / contribuição reversível / já soft-excluded → hard-delete.
    -- FK sale_order_strap_demands é ON DELETE SET NULL (22100).
    ...
    UPDATE public.sale_order_items i
       SET production_excluded_at = coalesce(i.production_excluded_at, now()),
           ...
           production_exclusion_reason = coalesce(
             ...
             'Removido na edicao do PV; compra de tira avancada preservada'
           ),
  ...
  IF cardinality(v_delete) > 0 THEN
    -- Cancela compra reversível ANTES do DELETE (BEFORE DELETE barra contribuição
    -- ativa). Mesmo contrato 22200/22900/23000, agora no caminho de hard-delete
    -- da 1ª remoção.
    WITH cancelled_purchases AS (
      UPDATE public.purchase_demand_contributions c
         SET status = 'cancelled',
             ...
             correlation_id = p_correlation_id,
             updated_at = now()
       WHERE c.sale_order_item_id = ANY(v_delete)
...
BEGIN
  ...
  IF position('pv_edit_hard_delete_first_removal_20270101026300' IN v_finalize) = 0 THEN
    RAISE EXCEPTION 'Guard: finalize sem marcador 26300 (hard-delete na 1ª remoção)';
  END IF;
  IF position('pv_edit_hard_delete_command_boundary_20270101023500' IN v_finalize) = 0 THEN
    RAISE EXCEPTION 'Guard: finalize perdeu marcador 23500 (command boundary)';
  END IF;
  IF position('pv_edit_detach_cancelled_po_fk_20270101022900' IN v_finalize) = 0 THEN
    RAISE EXCEPTION 'Guard: finalize perdeu marcador 22900 (detach OC)';
  END IF;
  IF position('pv_edit_delete_orphan_po_after_detach_20270101023000' IN v_finalize) = 0 THEN
    RAISE EXCEPTION 'Guard: finalize perdeu marcador 23000 (órfão OC)';
  END IF;
  IF position('compra de tira avancada preservada' IN v_finalize) = 0 THEN
    RAISE EXCEPTION 'Guard: soft-preserve nao cita compra avancada';
  END IF;
  -- Motivo antigo = classificação "qualquer demanda → preserve". Não pode voltar.
  -- Concatena pra o literal completo não aparecer no arquivo do guard.
  IF position(('demanda de tira historica' || ' preservada') IN v_finalize) > 0 THEN
    RAISE EXCEPTION 'Guard: finalize ainda soft-preserva por existencia de strap_demands';
  END IF;
END;
$$;
